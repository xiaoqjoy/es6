<template>
  <div>
    <!-- 添加患者页面 -->
    <div class="positionContent" id="addPatient">
      <div>
      ...
      <div class="container-base-info contentBorder">
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-position="top" label-width="110px" class="demo-ruleForm1 patientInfo" onsubmit="return false;">
          <el-form-item label="手机号" prop="phone">
            <el-input v-model.trim="ruleForm.phone" size="medium" @input="queryPatient" @keyup.enter.native="queryPatient" :maxlength="11"></el-input>
          </el-form-item>
          <el-form-item label="姓名" prop="patientName">
            <el-input v-model.trim="ruleForm.patientName" maxlength="50" size="medium"></el-input>
          </el-form-item>
          <el-form-item label="身份证号" prop="idCard">
            <el-input v-model.trim="ruleForm.idCard" size="medium" clearable maxlength="18" @change="handleBirthdayAgeSex"></el-input>
          </el-form-item>
          ...
          <el-form-item v-if="isAdmin" label="选择科室" prop="departmentId">
            <el-select v-model="ruleForm.departmentId" placeholder="请选择" size="medium" @change="changeDepart">
              <el-option v-for="(item,index) in DepartmentList" :key="item.departmentId" :label="item.name" :value="item.departmentId">
                {{ item.name }}
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="isAdmin" label="选择医生" prop="doctorId">
            <el-select v-model="ruleForm.doctorId" placeholder="请选择" size="medium">
              <el-option v-for="(item,index) in doctorList" :key="item.userId" :label="item.nickname" :value="item.userId">
                {{item.nickname}}
              </el-option>
            </el-select>
          </el-form-item>
          <el-form-item v-if="isAdmin" label="选择护士" prop="nurseId">
            <el-select v-model="ruleForm.nurseId" placeholder="请选择" size="medium">
              <el-option v-for="(item,index) in nurseList" :key="item.userId" :label="item.nickname" :value="item.userId">
                {{item.nickname}}
              </el-option>
            </el-select>
          </el-form-item>
      ...
      <div class="container-base-info contentBorder">
        ...
        <el-form :model="casesInfo" ref="casesInfo" label-position="top" label-width="110px" class="demo-ruleForm1 casesInfo" onsubmit="return false;">
          ...
          <!--
          <div>
            <el-form-item label="最近一次血压（mmHg）" style="width: 33%!important;">
              <el-col class="grid2">
                <el-input v-model="casesInfo.systolicPressure" placeholder="收缩压" size="medium" ></el-input>
              </el-col>
              <el-col class="gutter">/</el-col>
              <el-col class="grid2">
                <el-input v-model="casesInfo.diastolicPressure" placeholder="舒张压" size="medium" ></el-input>
              </el-col>
            </el-form-item>
            <el-form-item label="心率（p/min）" style="width: 33%!important;">
              <el-col class="grid2">
                <el-input v-model="casesInfo.heartRate" size="medium" ></el-input>
              </el-col>
            </el-form-item>
          </div>
          -->
      ...
      <!--
      <div class="container-base-info contentBorder">
        <label class="label-base-info">实验检测</label>
        <div class="line-divider"></div>
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-position="top" label-width="110px" class="demo-ruleForm1 detection" onsubmit="return false;">
            <el-form-item label="糖化血红蛋白（%）">
              <el-input v-model="ruleForm.GlycatedHemoglobin" size="medium" ></el-input>
              <span class="note">正常范围：4~6</span>
            </el-form-item>
            <el-form-item label="空腹血糖（mmol/L）">
              <el-input v-model="ruleForm.bloodSugar" size="medium" ></el-input>
              <span class="note">正常范围：3.9~6.1</span>
            </el-form-item>
            <el-form-item label="葡萄糖耐受实验2h血糖（mmol/L）">
              <el-input v-model="ruleForm.glucoseTest" size="medium" ></el-input>
    ...
    <el-dialog title="重复提醒" :visible.sync="reTip" :append-to-body="true" width="300px" :close-on-click-modal="false" @close="repeatTipClose">
      <div>
        <p><span>该手机号<span style="color: #267bf7;"> {{ruleForm.phone}} </span>已经创建过档案，点击确定获取档案信息</span>
        </p>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="fillPatientInfo">确定</el-button>
        <el-button @click="repeatTipClose">取消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
<script>
  ...
  export default {
    ...
    created() {
      ...
      if (user.roleType == '3') { //管理员
        ...
        if (user.departmentId) {
          this.ruleForm.departmentId = user.departmentId;
          this.changeDepart(user.departmentId); //根据科室id获取医生、护士列表
          this.$nextTick(function () {
            this.ruleForm.doctorId = user.userId;
          })
        }
      }
    ...
    methods: {
      ...
      getPatientInfo(data) {
        ...
        that.$api.getPatientInfo(data, res => {
          if (res.status === 200) {
            ...
            let departmentId = dt.doctorDepartmentId !== null ? dt.doctorDepartmentId : ''; //科室id
            if (departmentId) {
              that.ruleForm.departmentId = departmentId;
              that.changeDepart(departmentId); //根据科室id获取医生、护士列表
              that.$nextTick(function () {
                that.ruleForm.doctorId = dt.doctorId;
              })
            }
            
            let patientBaseVo = dt.patientBaseVo;
            let patientSugarVo = dt.patientSugarVo;
      ...
      // 科室change事件（更换科室对应的医生与护士）
      changeDepart(id) {
        this.ruleForm.doctorId = '';
      ...
      getDepartmentList() {
        ...
        this.$api.queryDepartmentList(data, res => {
          if (res.data.code == 0) {
      ...
      queryPatient() {
        let vm = this;
        vm.ruleForm.phone = vm.ruleForm.phone.toString().replace(/\s/g, ""); //去空格
        if (vm.ruleForm.phone.length == 11) {
          // 根据输入信息搜索saas内的患者
          //vm.$api.getPatientByPhone({keyName: vm.ruleForm.phone},res=> {
          //  if (res.data.code == 0) {
          //    if (res.data.data.length > 0) {
          //      vm.$message.warning('该手机号已经在机构建档，不可重复建档！');
          //      vm.$refs['ruleForm'].resetFields();
          //    } else {
          //      vm.isExistPhone(); //验证公众号内是否存在
          //    }
          //  }
          //}, res => {
      ...
      isExistPhone() {
        ...
        vm.$api.byPhoneExist({
          ...
          if (res.data.code == 0) {
            vm.reTip = true;
            vm.customerId = res.data.data.customerId;
          }
        }, fail => {
          if (fail.data.code == -5) {
            console.log('手机号码没有在公众号注册');
          }
        })
      },
      //填充患者信息
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            if (vm.isAdmin) {
              //获取医生姓名
              if (!!vm.ruleForm.doctorId) {
                for (let i = 0, len = vm.doctorList.length; i < len; i++) {
                  if (vm.doctorList[i].userId == vm.ruleForm.doctorId) {
                    doctorName = vm.doctorList[i].nickname;
                    break;
                  }
                }
              }
              //获取护士姓名
              if (!!vm.ruleForm.nurseId) {
                for (let i = 0, len = vm.nurseList.length; i < len; i++) {
                  if (vm.nurseList[i].userId == vm.ruleForm.nurseId) {
                    nurseName = vm.nurseList[i].nickname;
                    break;
                  }
                }
              }
            ...
            data.departmentId = vm.ruleForm.departmentId; //科室
            data.doctorId = vm.ruleForm.doctorId; //医生
            data.nurseId = vm.ruleForm.nurseId; //护士
            data.doctorName = doctorName; //医生姓名
            data.nurseName = nurseName; //护士姓名
