<template>
  <div>
    ...
    <div class="container-root">
      ...
      <div>
        <!--<label class="label-patient-manager">患者管理</label>-->
        <router-link to="/patientManage" class="label-patient-manager">患者管理</router-link>
        <i class="el-breadcrumb__separator el-icon-arrow-right"></i>
        <label class="label-add-patient">新建患者</label>
      ...
      <div class="container-base-info contentBorder">
        ...
        <div style="width: 350px; float:right">
          <div class="container-search" style="position: relative;">
            <input id="edit-search" v-model="keyName" @keyup.enter='m_input' type="text" class="input-search" style="margin-left:0px;" placeholder="查询已有患者：手机号"/>
            <div class="btn-search" @click="m_input"></div>
            <!---->
            <div class="abouts" v-if="ifDataShow">
              <ul>
                <li v-for="a in mdatass" style="height: 36px;" class="hovers"
                  ...
                  <ol>
                    <li>{{a.patientName}}</li>
                    <li>{{a.sex==1 ? "男" : a.sex==2 ? "女" : a.sex==0 ? '未知' : '' }}</li>
                    <li>{{a.phone}}</li>
                  </ol>
                </li>
              </ul>
        ...
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="110px" class="demo-ruleForm"
          ...
          <el-form-item label="身高">
            <input id="edit-height" @blur="b_height" type="number" v-model="ruleForm.height" @input="mins_2"
                   style="width: 40px;"/> CM
          </el-form-item>
          <el-form-item label="体重">
            <input id="edit-weight" @blur="b_weight" type="number" v-model="ruleForm.weight" @input="mins_3"
                   style="width: 40px"/> KG
          </el-form-item>
          ...
          <el-form-item label="手机号" prop="phone">
            <el-input v-model="ruleForm.phone" :disabled="phoneDisabled" @blur="isExistPhone(ruleForm.phone)" :maxlength="11"></el-input>
          </el-form-item>
          ...
          <el-form-item label="职业" prop="">
            <!--<el-select v-model="ruleForm.Occupation" value-key="ruleForm.Occupation" placeholder="请选择" @change="OccupaChange">-->
            ...
            <!--<select v-model="ruleForm.Occupation">
                        <option v-for="option in Occpationss" v-bind:value="option.itemKey">
                          {{ option.itemValue }}
                        </option>
                      </select>  -->
          </el-form-item>
          ...
          <el-form-item label="家庭住址" prop="" style="width: 39%;">
            <div class="row">
              <div class="col">
                <select v-model="prov" @change="changeProv" style="margin-left: 0;">
        ...
        <el-form :model="healthInfor" :rules="rules" ref="healthInfor" label-width="100px" class="demo-healthInfor">
          <el-form-item label="疾病史" prop="" style="position: relative;">
            ...
            <el-button v-popover:popovers :disabled="JSON.parse(healthInfor.disease)">
              <el-input readonly type="textarea" :disabled="JSON.parse(healthInfor.disease)"
                        v-model="elses ? hDisease.toString() + ',' + elses : hDisease.toString()"
            ...
            <!--弹框-->
              <div class="diseases">
                ...
                <div>
                  ...
                  <div class="button_" style=" margin-left: 0; background-color: #F56C6C;" @click="cancel">取消</div>
                  <div class="button_" @click="sureS">确定</div>
                </div>
              </div>
            </el-dialog>    
          </el-form-item>
      ...
      <div class="container-base-info contentBorder" style="height:auto;overflow:hidden;">
        ...
        <el-form :model="HabitsCus" :rules="rules" ref="HabitsCus" label-width="100px" class="demo-HabitsCus">
          ...
          <el-form-item label="抽烟" prop="">
            ...
            <input id="edit-smoking-daily" :disabled="smokingStatusRadio==1 || smokingStatusRadio==2" @input="mins_1"
                   type="number" style="width: 50px" v-model="SmokVolume"/>
          ...
          <el-form-item label="饮酒" prop="">
            ...
            <el-select v-model="drinkTy_" placeholder="请选择" :disabled="drinkingStatusRadio==1"
                       style="width: 120px;margin-left: 30px;" @change="driChange">
              <el-option
            ...
            <input id="edit-drinking-daily" v-model="drinkVolume" @input="mins" type="number"
                   :disabled="drinkingStatusRadio==1" style="width: 50px;height: 39px;"/>
    ...
    <el-dialog title="重复提醒" :visible.sync="reTip" :append-to-body="true" width="28%" :close-on-click-modal="false">
      <div>
        <p>“<span style="color: #267bf7;">手机号<span>{{existPhone.phone}}</span></span>”已经在公众号创建过档案
          ，请确认是否输入有误。
          如果输入正确，请选择获取公众号档案或者是继续
          新建患者。</p>
      </div>
      <div slot="footer" class="dialog-footer">
        <el-button :disabled="clickDisabel" @click="takeSearch(existPhone.nickname,existPhone.customerId)">获取公众号档案</el-button>
        <el-button :disabled="clickDisabel" @click="continueAdd">继续新建患者</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  var cuGetPat = function (that, customerId, userCusId) {
    that.$api.cuGetPat({customerId:customerId, userCustomerId:userCusId},res=> {
      if (res.status === 200) {
        ...
        that.ruleForm.name = patientBaseVo.patientName !== null ? patientBaseVo.patientName : "";//姓名
        that.ruleForm.sex = patientBaseVo.sex !== null ? patientBaseVo.sex : "";//性别
        that.ruleForm.birth = patientBaseVo.birthday !== null ? patientBaseVo.birthday : "";//出生日期
        that.ruleForm.height = patientBaseVo.height !== null ? patientBaseVo.height : "";//身高
        that.height = patientBaseVo.height !== null ? patientBaseVo.height : "";
        that.ruleForm.weight = patientBaseVo.weight !== null ? patientBaseVo.weight : "";//体重
        that.weight = patientBaseVo.weight !== null ? patientBaseVo.weight : "";
        that.ruleForm.IDNum = patientBaseVo.idCard !== null ? patientBaseVo.idCard : "";//身份证
        that.ruleForm.phone = patientBaseVo.phone !== null ? patientBaseVo.phone : "";//手机号
        //省市区
        that.prov = patientBaseVo.province != null ? patientBaseVo.province : '北京';//
        that.city = patientBaseVo.city != null ? patientBaseVo.city : '北京';//
        that.district = patientBaseVo.district != null ? patientBaseVo.district : '东城区';//
        that.num++
        that.updateCity();
        that.updateDistrict();

        that.ruleForm.detail = patientBaseVo.address != null ? patientBaseVo.address : '';
        that.ruleForm.mar = patientBaseVo.maritalStatus !== null ? patientBaseVo.maritalStatus : "";//婚姻状态1未婚  2 已婚  3丧偶  4离婚
        //职业1国家机关、党群组织、企业、事业单位负责人 2专业技术人员 3办事人员和有关人员  4商业、服务业人员  5 农、林、牧、渔、水利业生产人员  6生产、运输设备操作人员及有关人员  7军人  8不便分类的其他从业人员
        that.ruleForm.Occupation = patientBaseVo.job !== null ? patientBaseVo.job : "";
        //BMI
        var bmi = that.getBMI();
        if (bmi === -1) {
        ...
        //住址
        //紧急联系人
        that.ruleForm.emCont = patientBaseVo.emergencyContact !== null ? patientBaseVo.emergencyContact : "";
        that.ruleForm.contactPhone = patientBaseVo.emergencyPhone !== null ? patientBaseVo.emergencyPhone : "";
        //TODO:设备编码、类型、并发症、确诊时间
        //$("#edit-device-code").val();
        //ruleForm.
        that.ruleForm.diseaseType = patientSugarVo.diabetesType !== null ? patientSugarVo.diabetesType : ''; //类型
        that.ruleForm.DeviceCod = patientSugarVo.deviceId !== null ? patientSugarVo.deviceId : ''; //设备编码
        that.ruleForm.Complica = patientSugarVo.complications !== null ? patientSugarVo.complications : ''; //并发症
        that.ruleForm.timeDiag = patientSugarVo.ensureDate !== null ? patientSugarVo.ensureDate : ''; //确诊时间
        //标签
        //过敏史
        //TODO:疾病史、现病史、现在治疗方式、过去治疗方式
        var _hDisease = patientBaseVo.diseasesHistory !== null && patientBaseVo.diseasesHistory !== '' ? patientBaseVo.diseasesHistory.split(",") : "";
        if (_hDisease) {
        ...
        //处理数据
        var _hFamily = patientBaseVo.familyHistory !== null && patientBaseVo.familyHistory !== '' ? patientBaseVo.familyHistory.split(",") : "";
        if (_hFamily) {
        ...
        //
        that.hThisDisease = patientBaseVo.historyOfPresentIllness !== null && patientBaseVo.historyOfPresentIllness !== '' ? patientBaseVo.historyOfPresentIllness : "";
        if (that.hThisDisease) {
          that.healthInfor.thisDisease = 'false';
        } else {
          that.healthInfor.thisDisease = 'true';
        }
        //
        that.hTreatMethod = patientBaseVo.nowTreatment !== null && patientBaseVo.nowTreatment !== '' ? patientBaseVo.nowTreatment.split(",") : "";
        ...
        that.checkList = patientBaseVo.nowTreatment !== null && patientBaseVo.nowTreatment !== '' ? patientBaseVo.nowTreatment.split(",") : "";
        if (that.hTreatMethod) {
          that.healthInfor.TreatMethod = 'false';
        } else {
          that.healthInfor.TreatMethod = 'true';
        }
        //
        that.hAllergy = patientBaseVo.allergicHistory !== null && patientBaseVo.allergicHistory !== '' ? patientBaseVo.allergicHistory : "";
        if (that.hAllergy) {
          that.healthInfor.allergy = 'false';
        } else {
          that.healthInfor.allergy = 'true';
        }
        //
        that.hPastTreatment = patientBaseVo.pastTreatment !== null && patientBaseVo.pastTreatment !== '' ? patientBaseVo.pastTreatment : "";
        if (that.hPastTreatment) {
          that.healthInfor.PastTreatment = 'false';
        } else {
          that.healthInfor.PastTreatment = 'true';
        }
        //劳动强度
        that.labourIntensityRadio = patientBaseVo.labourIntensity !== null ? patientBaseVo.labourIntensity : "";
        //吸烟情况
        that.smokingStatusRadio = patientBaseVo.smokingStatus !== null ? patientBaseVo.smokingStatus : "";
        //吸烟频率
        that.SmokVolume = patientBaseVo.smokingDaily !== null ? patientBaseVo.smokingDaily : "";
        //饮酒
        that.drinkingStatusRadio = patientBaseVo.drinkingStatus !== null ? patientBaseVo.drinkingStatus : "";
        //          饮酒类型
        that.drinkTy_ = patientBaseVo.drinkingType !== null ? patientBaseVo.drinkingType : "";
        //饮酒量
        that.drinkVolume = patientBaseVo.drinkingDaily !== null ? patientBaseVo.drinkingDaily : "";
        //锻炼情况
        that.exerciseFrequencyRadio = patientBaseVo.exerciseFrequency !== null ? patientBaseVo.exerciseFrequency : "";
        //
        that.sleepStatusRadio = patientBaseVo.sleepStatus !== null ? patientBaseVo.sleepStatus : "";

        var foodHabitRadio_ = patientBaseVo.foodHabit !== null ? patientBaseVo.foodHabit.split(",") : "";
        if (foodHabitRadio_) {
        ...
        that.OrdMood = patientBaseVo.peacetimeEmotion !== null ? patientBaseVo.peacetimeEmotion : "";

        that.Remarks = patientBaseVo.remark !== null ? patientBaseVo.remark : "";

        that.HealthRevi = patientBaseVo.healthReview !== null ? patientBaseVo.healthReview : "";

        that.searchIsShow = false;//隐藏搜索结果框
      }
    }).catch(function (err) {
  ...
  export default {
      ...
      return {
        ...
        rules: {
          ...
          name: [
            ...
            {validator: funBus.validateIsNull, trigger: 'change'}
          ],
          sex: [
    ...
    methods: {
      ...
      //是否在公众号存在号码
      isExistPhone(phone){
        if(phone.toString().replace(/(^\s*)|(\s*$)/g, "")!='' && /^1[345678]\d{9}$/.test(phone)){
          let that = this
          this.$api.byPhoneExist({phone:phone}, res =>{
            if(res.data.code == 0){
              that.reTip = true
              that.existPhone = res.data.data
            }
          },fail=>{
            if(fail.data.code == -5){
              // that.$message(fail.data.errMsg)
            }
          })
        }
      },
      ...
      mins() {
        var that = this;
        this.drinkVolume = that.drinkVolume.replace(/[^\d]/g, '')
      },
      mins_1() {
        var that = this;
...
//          console.log(n)
      ...
      takeSearch(name, customerId) {
        ...
        // }
        var userCusId = "";
        cuGetPat(this, customerId, userCusId);
      },
      m_input() {
        var that = this;
        if (that.keyName.trim() && /^1[345678]\d{9}$/.test(that.keyName)) {
          that.$api.queryName_({keyName:that.keyName.trim()},res=> {
            if (res.status == 200) {
              if (res.data.data) {
        ...
        }else if(!/^1[345678]\d{9}$/.test(that.keyName)){
...
//          数据处理
      ...
      handleInputConfirm() {
        ...
        if (inputValue) {
          ...
          this.patientTags.forEach(function (i) {
            if (i === inputValue) {
              this.$message('该标签名字已存在！')
            }
          });
          /*获取session信息*/
      ...
      sureS() {
        var close = true;
        //判断饮食控制 清空
        //console.log(this.DietControl)
        //console.log(this.Oral)
        //console.log(this.Insul)
        ...
        if (this.Oral) {
          for (var i = 0; i < this.OralMedi.length; i++) {
            ...
            //console.log(this.OralMedi[i].warn)
...
//          }
          ...
          if (valid) {
            ...
            var data = {
              patientBaseVo: {
                ...
                job: that.Occupa_,//职业1国家机关、党群组织、企业、事业单位负责人 2专业技术人员 3办事人员和有关人员 4商业、服务业人员 5 农、林、牧、渔、水利业生产人员 6生产、运输设备操作人员及有关人员 7军人 8不便分类的其他从业人员 ，
                bmi: that.ruleForm.BMI == "请完善身高体重" ? "" : JSON.parse(that.ruleForm.BMI),
                province: that.prov,//
                city: that.city,//
                district: that.district,//
                address: that.ruleForm.detail, //家庭住址 省 市 区/县 地址,70.
                ...
                drinkingType: that.drinkKey,//饮酒类型,
                drinkingDaily: that.drinkVolume ? JSON.parse(that.drinkVolume) : '',//饮酒量,
                foodHabit: that.foodKey ? that.foodKey.toString() : '',// 饮食习惯
            ...
            that.loading = true;
            that.$api.customerAdd(data, res=> { // 成功回调
              if (res.status == 200) {
      ...
      onSearchItemClick(item) {
        var customerId = this.cusID;
        var that = this;
//      var data = {"customerId": customerId}
        var userCusId = '';
        cuGetPat(that, customerId, userCusId);
      },
      getBMI: function () {
...
//                      console.log(that.drinkingTypes)
          }
...
<style>
.marry .el-radio .el-radio__label {
