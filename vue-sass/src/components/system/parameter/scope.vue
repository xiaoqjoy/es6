<template>
  <div class="parameter">
    <div class="parameterScope">
      <el-table
        :data="tableData"
        style="width: 100%;border-left:1px solid rgb(235,238,245);border-right: 1px solid  rgb(235,238,245);border-top:1px solid rgb(235,238,245);  ">
        <el-table-column
          fixed
          prop="name"
          align="center"
    ...
    <el-dialog width="20%" title="血糖正常范围编辑" :visible.sync="dialogFormVisible">
      <el-form :model="form" label-position="left" :rules="rules" ref="form" class="demo-ruleForm">
        ...
        <el-form-item label="正常范围最小值" :label-width="formLabelWidth" prop="minV">
          <el-input v-model="form.minV" style="width:50%" auto-complete="off" /> &nbsp;mmol/L
        </el-form-item>
        <el-form-item label="正常范围最大值" :label-width="formLabelWidth" prop="maxV">
          <el-input v-model="form.maxV" style="width:50%" auto-complete="off" /> &nbsp;mmol/L
        </el-form-item>
      </el-form>
        ...
        data() {
          ...
          var valimaxV = (rule, value, callback) => {
            ...
            return {
              ...
              rules: {
                minV:[
                  {required: true, message: '请输入正常范围最小值(0.1~10.0)', trigger: 'blur'},
                  {validator: valiminV, trigger: 'blur'}
                ],
                maxV:[
                  {required: true, message: '请输入正常范围最大值(3.0~20.0)', trigger: 'blur'},
                  {validator: valimaxV, trigger: 'blur'}
                ]
              },
            ...
            this.$refs[formName].validate((valid) => {
                if (valid) {
                  let data = {
                    ...
                    minValue:this.form.minV,
                    maxValue:this.form.maxV
                  }
