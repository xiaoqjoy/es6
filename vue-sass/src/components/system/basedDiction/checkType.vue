<template>
  <div class="checkType">
    ...
    <el-table
      ref="multipleTable"
      :data="checkType.slice((currentPage4-1)*pagesize,currentPage4*pagesize)"
      tooltip-effect="dark"
      style="width: 100%"
...
<script>
  ...
  export default {
    ...
    methods: {
      //新增编辑报告类型
      newAdd() {
        this.$emit("newChecType", "true", "新增");
      },
      handleClick(row) {
        // console.log(row);
        this.$emit("newChecType", "true", "编辑", row);
      },
      //删除报告类型
      intermeddle(row) {
        this.$emit("delChecType", "true", row)
      },
