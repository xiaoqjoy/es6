<template>
  <div class="meansInterven">
    <div class="manage_R">
      <div class="newAdd_R ipts" @click="newAdd_R()">新增方式</div>
    </div>
    <!--表格-->
    <el-table
      ref="multipleTable"
      :data="meansInterven.slice((currentPage4-1)*pagesize,currentPage4*pagesize)"
      tooltip-effect="dark"
      style="width: 100%"
...
<script>
  ...
  export default {
    ...
    methods: {
      //新增方式
      newAdd_R() {
        this.$emit("newWays", "true", "新增");
      },
      //删除方式
      intermeddle(row) {
        this.$emit("delWays", "true", row)
      },
      handleClick(row) {
        this.$emit("newWays", "true", "编辑", row);
      },
