<template>
  <div class="interType">
    <div class="manage_R">
      <div class="newAdd_R ipts" @click="newAdd()">新增类型</div>
    </div>
    <!--表格-->
    <el-table
      ref="multipleTable"
      :data="interType.slice((currentPage4-1)*pagesize,currentPage4*pagesize)"
      tooltip-effect="dark"
      style="width: 100%"
...
//	        currentPage3: 5,
    ...
    methods: {
      //新增编辑干预类型
      newAdd() {
        this.$emit("newInterType", "true", "新增");
      },
      handleClick(row) {
        this.$emit("newInterType", "true", "编辑", row);
      },
      //删除角色
      intermeddle(row) {
        this.$emit("delIntType", "true", row)
      },
