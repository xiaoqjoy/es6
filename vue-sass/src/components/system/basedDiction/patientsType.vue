<template>
  <div class="patientsType">
    ...
    <el-table
      ref="multipleTable"
      :data="patientsType.slice((currentPage4-1)*pagesize,currentPage4*pagesize)"
      tooltip-effect="dark"
      style="width: 100%"
...
<script>
  ...
  export default {
    ...
    methods: {
      //新增类型
      newAdd() {
        this.$emit("newType", "true", "新增");
      },
      // 编辑
      handleClick(row) {
        this.$emit("newType", "true", "编辑", row);
      },
      //删除
      intermeddle(row) {
        this.$emit("delType", "true", row)
      },
