    ...
    <div class="product_cart_main">
          ...
          <div class="md-example-child md-example-child-date-picker md-example-child-date-picker-3">
            ...
            <div class="product_cart_item_box">
              <div class="product_cart_item">
                ...
                <div class="warranties_input_box">
                  <span class="warranties_input">{{myData.name}}</span>
                </div>
              </div>
            </div>
            <!-- 证件号 -->
            <div class="product_cart_item_box">
              <div class="product_cart_item">
                ...
                <!-- item右边 -->
                <div class="warranties_input_box">
                  <span class="warranties_input">{{myName}}</span>
                </div>
              </div>
            </div>
            <!-- 证件号 -->
            <div class="product_cart_item_box">
              <div class="product_cart_item">
                ...
                <div class="warranties_input_box">
                  ...
                  >{{myNumbers.replace(/\s/g, '').replace(/(.{4})/g, "$1 ")}}</span>
                </div>
              </div>
            </div>
            <!-- 手机号 -->
      ...
      <div class="product_cart_submit">
        <div class="p_i_s_main">
          <button class="submit" @click="submit" ref="productWarrantiesButton">下一步</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getTheParam() {
      this.myData = JSON.parse(localStorage.getItem("myRepayment")) || "";
      this.myName = JSON.parse(localStorage.getItem("cardNameName")) || "";
      this.myNumbers =
        JSON.parse(localStorage.getItem("cardNumbersNumbers")) || "";
      this.accountOwner =
        JSON.parse(localStorage.getItem("accountOwner")) || "";
    },
    // 手机号正则校验
    verifyMobile() {
    ...
    captchaInterface() {
      axios({
        ...
        data: {
          ...
          idNo: this.myData.idNo, // 身份证号
          name: this.myData.name, // 姓名
          accountNo: this.myNumbers // 银行卡号
        }
      }).then(res => {
    ...
    // 认证按钮
    submit() {
      if (this.phoneNumber == "" || this.messageCode == "") {
        Toast({
          message: "手机号/验证码不能为空",
          position: "middle",
          duration: 5000
