 ...
 * @Author: your name
 * @Date: 2019-08-19 14:17:17
 * @LastEditTime: 2019-08-22 16:01:52
     ...
     <div class="product_cart_main">
       ...
       <div class="product_cart_userInfo">
         <div class="debtHonor" v-for="(item,index) in loanList" :key="index">
           <div class="top">
            <div class="debt_i_com">{{item.bankName}}</div>
            <div class="debt_i_right">{{item.accountNo?item.accountNo.replace(/\s/g, '').replace(/(.{4})/g, "$1 "):''}}</div>
           </div>
            <div class="button_bottom">
                <button @click="jumpRepayment(item.bankName, item.accountNo)">绑定</button>
                <!-- <span v-else class="button_bottom_span">已绑定</span> -->
            </div>
         </div>
       </div>
...
export default {
  ...
  mounted(){
    // console.log('1111',JSON.parse(localStorage.getItem("useTokenBank")));
  ...
  methods: {
    ...
    // 银行卡四位一个空格显示
    // setNum(data) {
    //     var  bankCard = data.replace(/(\d{4})(?=\d)/g, '$1');
    //     return bankCard;
    // },
    // 跳转到银行卡签约页面
    jumpRepayment(cardName, cardNumbers) {
    ...
    myInterface() {
      axios({
        ...
        .then((res) => {
          this.loanList = res.data.data.cardInfoDTOList;
