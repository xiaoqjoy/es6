  ...
  <section class="follow-box dialog-box">
    ...
    <div class="follow-table border-ddd">
      <el-table :data="followData"
        ...
        <el-table-column prop="followPersonName"
          label="跟进人"
          align="left"
          width="180">
        </el-table-column>
        <el-table-column prop="followTimeDesc"
          label="跟进时间"
          align="left"
          width="180">
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    requestData() {
      this.loading = true;
      this.$axios.get(
        '/security/room/manage/queryFollowInfo',
        {
          params: {
            id: this.id
          }
    ...
    handleSubmit() {
      this.loading = true;
      this.$axios.post(
        '/security/room/manage/addFollowInfo',
        {
          followInfo: this.followInfo,
          followTime: (new Date()).valueOf(),
          roomId: this.id,
          followPersonName: this.bdName
        }
      ).then((res) => {
        this.loading = false;
        if (res.data.status === 'C0000' && res.data.result === 'C0000') {
          this.$message.success('操作成功');
          this.followInfo = '';
          this.requestData();
          if (this.activeIndex !== 'COMMIT_ROOM') {
