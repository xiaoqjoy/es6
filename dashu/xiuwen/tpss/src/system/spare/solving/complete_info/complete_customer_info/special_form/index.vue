    ...
    <div class="special_form-page">
      <el-form
        ...
        <el-form-item
          ...
          <el-select
            :disabled="relativeFn(relativeData[item.orgItemCode], item.orgItemCode) || hasSubmit"
          ...
          <el-switch
            ...
            :disabled="relativeFn(relativeData[item.orgItemCode], item.orgItemCode) || hasSubmit"></el-switch>
          <el-select
            clearable
            multiple
    ...
    methods: {
      dellArr() {
        this.renderArr.forEach((v) => {
          ...
          // 设置表单规则
          v.ruleJson = JSON.parse(v.ruleJson);
          ...
          if (v.ruleJson.funs.length > 0) {
            v.ruleJson.funs.forEach(k => {
              ...
              obj.validator = (new Function('rule', 'value', 'callback', k.fun)).bind(this);
          ...
          if (v.fieldType === 'select' || v.fieldType === 'multiSelect') {
            ...
            this.getLibraryApi(v.orgItemCode)
              .then(data => {
          ...
          if (v.fieldType === 'cascader') {
            ...
            this.getAllLibraryApi(v.orgItemCode)
              .then(data => {
                this.$set(this.library, v.orgItemCode, data);
          ...
          // control: [{relativeCode: "控制该字段的字段名"， disabled: ['控制字段出现哪些值导致该字段不可用']}]
      ...
      getLibraryApi(name) {
        return new Promise((resolve) => {
          this.$MyFetch.get(spareApi.commonSelect, {categoryCode: name, getAll: this.hasSubmit})
            .then((data = {}) => {
              resolve(data);
            })
            .catch(err => {
      ...
      getAllLibraryApi(name) {
        return new Promise((resolve) => {
          this.$MyFetch.get(spareApi.commonSelectAll, {categoryCode: name, getAll: this.hasSubmit})
            .then((data = {}) => {
              resolve(data);
            })
            .catch(err => {
