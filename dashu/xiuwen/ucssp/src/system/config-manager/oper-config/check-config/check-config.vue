<template>
  <div>
    <div class="content">
      ...
      <el-select class="select_72" @change="defaultList_" v-model="value3"  placeholder="电核维度">
    ...
    <addCheckConfig :cause_x1="cause_" :cause_x="cause" :value3="value3" :for="for_" v-if="alert_" @clsoe_windowAlert='close_1' />
    <editCheckConfig :cause_x1="cause_" :cause_x="cause" :current_list='currentList' :list_="list" :value3="value3" v-if="alert_e" @clsoe_windowAlert_e='close_1_e' />
    ...
    <whetherSaveX :value3="value3" :cause_="cause_" :cause_x="cause" :msg="msg" v-if="toSaveH" @clsoe_sf='console_s' />
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      selectedRow: [], // 复选框是否选中
      list_input: [
        { name: "自雇人士", id: "phone_check_code_self_employed" },
        { name: "授薪人士", id: "phone_check_code_salary" }
      ],
      pageConfig: {
        account: 0, // 总条数
        pageSize: 10, // 当前多少条数据
  ...
  methods: {
    ...
    jump(pageNum) {
      ...
      if (this.value3 === "phone_check_code_salary") {
        ...
        this.cause_(this.pageConfig.currentPage);
      }
    },
    cause(pageNum) {
      let url = `${
        configApi.levelone_levelTwo
      }/${"phone_check_code_salary"}/list`;
      let data = {
        pageNum: pageNum, // 当前页
    ...
    deadText() {
      if (this.value3 === "phone_check_code_salary") {
        ...
        }/${"phone_check_code_salary"}/changeStatus`;
        this.$MyFetch
          .post(url, data)
          .then((data = {}) => {
            console.log(data);
        ...
        let url = `${
          configApi.levelone_changeStatus
        }/${"phone_check_code_self_employed"}/changeStatus`;
        this.$MyFetch
          .post(url, data)
          .then((data = {}) => {
            console.log(data);
