...
export default {
  ...
  mounted() {
    // this.importPayInteFineAdjustList();
  },
  methods: {
    // 一般还款查询总单接口
    importPayInteFineAdjustList(res = {}, current = 1) {
      let url = counterDealApi.importPayInteFineAdjustList + "?current=" + current + "&size=10";
    ...
    jump(pageNum) {
      ...
      this.importPayInteFineAdjustList(queryData, pageNum);
    },
    // 清空按钮
    empty() {
      this.loanNo = '';
    ...
    refer() {
      ...
      this.importPayInteFineAdjustList(queryData);
    },
    // 关闭弹窗
    close() {
