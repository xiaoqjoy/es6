<template>
    ...
    <!--退回 再次确认组件 -->
    <reconfirmHint v-if="delete_hint" massage="是否退回选中的申请？" @close="delete_close" @backFun="affirm_delete_fun"></reconfirmHint>
    <!--记账 再次确认组件 -->
    <reconfirmHint v-if="submit_hint" massage="是否对选中的申请进行记账？" @close="sumit_close" @backFun="affirm_submit_fun"></reconfirmHint>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        SingleRepaymentFun() {
            ...
            this.SingleRepayment(res, current);
        },
        // 一般还款查询总单接口
        ...
        transPayInfo(res) {
            return new Promise((resolve, reject) => {
                this.$MyFetch.post(counterDealApi.transPayInfo, res)
                    .then((data = {}) => {
                        resolve(data);
                    })
                    .catch((err) => {
        ...
        affirm_submit_fun() {
            this.submit_hint = false;
            console.log(this.backTallyData);
            // 单笔还款记账
            this.transPayInfo(this.backTallyData)
                .then((data) => {
                    // 查询总单方法
                    this.SingleRepaymentFun();
                })
        ...
        affirm_delete_fun() {
            ...
            console.log(this.backTallyData);
            // 单笔还款复核退回（支持批量/单笔）
            this.reviewReturn(this.backTallyData)
                .then((data) => {
                    // 查询总单方法
                    this.SingleRepaymentFun();
                })
        ...
        detail_alert(res) {
            ...
            console.log(res, this.tetail_data, '明细.');
        },
        // 选择框选中
        handleSelectionChange (data) {
            // 清空'退回/记账'的参数
            this.backTallyData.objectNoList = [];
            data.forEach(element => {
                let obj = {};
                // 获取选中的交易号,并push到'退回/记账'的参数列表去;
                obj.transSerialNo = element.trans_serial_no;
                this.backTallyData.objectNoList.push(element.trans_serial_no);
            });
            console.log(this.backTallyData, 'backTallyData');
        },
        ...
        retrun_fun() {
            console.log('退回');
            if (!this.backTallyData.objectNoList[0]) {
                this.$message('请至少选择一条数据退回');
            } else {
        ...
        refer() {
           ...
           let searchData = JSON.parse(JSON.stringify(this.searchData));
            searchData = this.loanTransCreated_filter(searchData);
           console.log(this.searchData, searchData, "操作人过滤后的searchdata");
