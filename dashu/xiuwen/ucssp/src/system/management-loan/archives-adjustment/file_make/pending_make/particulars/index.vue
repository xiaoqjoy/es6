      ...
      <div class="dialog-box__bottom">
        <div class="tables">
          ...
          <div class="box">
            ...
            <div class="content grid-content"><span class="left">客户姓名：</span><span>{{compile_data.custName || ""}}</span></div>
...
export default {
  ...
  methods: {
    // 关闭详情
    close_() {
      let url = loanApi.archives_cancel;
      let data = {
        operateType: "0", // 操作类型(0制作|1复核))
        paperArchivesId: this.compile_data.paperArchivesId // 档案编号
      };
      this.$MyFetch
        .get(url, data)
        .then((data = {}) => {
    ...
    // 提交接口
    finish() {
      let url = `${loanApi.archives_commit}?operateType=0&paperArchivesId=${
        this.compile_data.paperArchivesId
      }`;
      // console.log(data);
      this.$MyFetch
        .post(url)
