      ...
      <el-form :model="customerInfo"
        ...
        <div class="form__block bg__white border__radius-5" >
          ...
          <div class="dialog-box__form__group">
            ...
            <el-form-item label="婚姻状况" prop="maritalStatusCode" class="width355">
              <el-select v-model="customerInfo.maritalStatusCode" placeholder="请选择" :disabled="!!hasSubmit">
                <el-option
                          ...
                          <el-form :model="policyEdit"
                            ...
                            <div class="table__tr__bottom-list" >
                              ...
                              <el-form-item label="" prop="mainRiskName" class="table__form-item">
                                <el-input v-model="policyEdit.mainRiskName" maxlength="20"></el-input>
                              </el-form-item>
                  ...
                  <el-form :model="organizationTax"
                    ...
                    <div class="tax_box">
                      ...
                      <el-form-item class="btn_inline">
                        <el-button size="small" class="query_btn query_auth" @click="queryAuthorizationInfo" :disabled="queryImpowerBtnStatus || impowerFlag" v-if="!hasSubmit">查询授权结果</el-button>
        ...
        <div class="form__bottom clearfix">
          <el-button
            ...
            :disabled="hasSubmit">暂存</el-button>
          <el-button
            :loading="isSubmit"
            v-if="!isReadonly"
            type="primary"
...
<script type="text/ecmascript-6">
  ...
  export default {
      ...
      created() {
        ...
        this.getAreaList();
        this.getHasSubmit();
        let selectArr = ['insurance_company', 'marital_status_code', 'career_type_code', 'house_condition_code', 'share_percent', 'ins_payment_mode_code', 'estate_type_code', 'certificate_type_code', 'car_loan_status_code', 'gender_code'];
      ...
      methods: {
        ...
        getInfo(id) {
          this.$MyFetch.get(spareApi.solving.completeInfo.completeProduct.getInfo + id)
            .then((data = {}) => {
              ...
              data.registeredAddressCode = data.registeredAddressCode ? this.getAddressArr(data.registeredAddressCode, this.options) : this.customerInfo.registeredAddressCode;
              data.houseAddressCode = data.houseAddressCode ? this.getAddressArr(data.houseAddressCode, this.options) : this.customerInfo.houseAddressCode;
              data.workAddressCode = data.workAddressCode ? this.getAddressArr(data.workAddressCode, this.options) : this.customerInfo.workAddressCode;
              data.insurancePolicyInd = data.insurancePolicyInd ? data.insurancePolicyInd : false;
              data.estateInd = data.estateInd ? data.estateInd : false;
              data.carloanInd = data.carloanInd ? data.carloanInd : false;
              data.genderCode = data.genderCode ? data.genderCode : this.dCode.gendeTypeCode.M;
              data.organizationTaxInd = data.organizationTaxInd || false;
              data.localEstateInd = data.localEstateInd ? data.localEstateInd : false;
              data.houseFund6mInd = data.houseFund6mInd ? data.houseFund6mInd : false;
              data.registeredInd = data.registeredInd ? data.registeredInd : false;
              this.estateLimitCnt = parseInt(data.estateLimitCnt);
              if (data.foundDate) {
                data.foundDate = ((new Date(data.foundDate.replace(/-/g, '/'))).getTime()) ? (new Date(data.foundDate.replace(/-/g, '/'))).getTime() : 1;
              }
        ...
        submitCustomerApi(data) {
          this.$MyFetch.post(spareApi.solving.completeInfo.completeProduct.submitInfo, data, { timeout: 30000 })
            .then(() => {
        ...
        saveAll() {
          ...
          customerInfo.registeredAddressCode = this.customerInfo.registeredAddressCode[this.customerInfo.registeredAddressCode.length - 1] ? this.customerInfo.registeredAddressCode[this.customerInfo.registeredAddressCode.length - 1] : null;
          customerInfo.houseAddressCode = this.customerInfo.houseAddressCode[this.customerInfo.houseAddressCode.length - 1] ? this.customerInfo.houseAddressCode[this.customerInfo.houseAddressCode.length - 1] : null;
          customerInfo.workAddressCode = this.customerInfo.workAddressCode[this.customerInfo.workAddressCode.length - 1] ? this.customerInfo.workAddressCode[this.customerInfo.workAddressCode.length - 1] : null;
          // taxLoanPrepareDto
        ...
        addPolicy() {
          ...
          this.$refs.policyAddForm.validate((valid) => {
            if (valid) {
              ...
              this.$set(this.policyEdit, 'holderSwitchInd', this.policyEdit.holderSwitchInd ? this.policyEdit.holderSwitchInd : false);
        ...
        queryAuthorizationInfo() {
          ...
          if (this.saveStatus) {
            // 纳税人识别码变了,提醒用户保存
            this.confirmFn('纳税人识别码或税务接入机构被更改过，请重新保存后再获取授权信息');
            return false;
          }
