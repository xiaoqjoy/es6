<template>
  <div class="survey_config_list">
    ...
    <div class="table_box">
      <!-- 表格数据 -->
      <el-table class="feild_table" :data="pageData.list" style="100%" @row-dblclick="jumpToSecond">
        <el-table-column label="序号">
          <template slot-scope="scope"><span>{{scope.$index+1}} </span></template>
        </el-table-column>
        <el-table-column prop="itemCode" label="一级行业编号"></el-table-column>
        <el-table-column prop="itemDesc" label="一级行业名称"></el-table-column>
    ...
    <el-dialog class="dialog" :title="configDialog.title" :visible.sync="configDialog.dialogVisible" :before-close="handleClose" width="500px" center>
      <el-form class="search" :inline="true" :model="configInfo" :rules="configRule" ref="configInfo" label-position="top">
        <div class="dialog_con">
          <div class="top_conten"><span class="top_conten_title">一级行业编号：</span><span class="top_content_font">{{configInfo.itemCode}}</span></div>
        </div>
        <el-form-item label="一级行业名称" prop="itemDesc" placeholder="请填写商圈名称" class="width225">
...
export default {
  ...
  methods: {
    ...
    addConfig() {
      ...
      this.itemCodeNumber = 'HY' + time.replace(/\D/g, '') + parseInt((Math.random() * 9 + 1) * 1000);
      this.configInfo = {
