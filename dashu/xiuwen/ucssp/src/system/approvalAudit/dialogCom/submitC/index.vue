...
import {aprAudApi} from '../../js/server.js';
  export default {
    ...
    methods: {
      turnBorrow(tOrF) {
        if (this.appOrAud) {
            this.url = `${aprAudApi.taskDetail.submitTwiceOper}/${this.$route.query.appID}`;
          } else {
            this.url = `${aprAudApi.aud.taskDetail.submitTwiceOper}/${this.$route.query.appID}`;
          };
         let data = {
    ...
    created() {
      console.log(this.submitOper.opinionResultCode, this.opinionTxt, this.rejectReasonCode);
    }
  };
</script>
