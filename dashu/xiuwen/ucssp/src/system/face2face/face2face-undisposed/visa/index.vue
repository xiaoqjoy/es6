  ...
  methods: {
      ...
      switch (this.currentStep) {
        ...
        case 2:
          ...
          this.$nextTick(() => {
            ...
            // 录像初始化
            this.$refs["video"].$emit("showVideo");
            // console.log(this.$refs["video"].isVideoSuccess);
          });
          // 查询面签意见话术库
          this.faceCheckOpinionLibrary();
          break;
    ...
    faceCheckOpinionLibrary() {
      this.$MyFetch
        ...
        .then(res => {
          // console.log(res);
          this.checkDimens = res.opinionLibrary;
        })
        .catch(err => {
          console.error(err);
          this.confirmFn(err.message, "error");
    ...
    faceCheck_submit() {
      ...
      // 判断是否录像成功，不成功则不进行面核操作
      // if (!this.$refs["video"].isVideoSuccess) {
      //   this.confirmFn("您还没有录像，请进行录像再进行提交面核操作", "error");
      //   return;
      // }
      // return false;
    ...
    //  return;
        ...
        const opinionItemListLength = this.checkDimens[checkDimensLength - 1].opinionItemList.length;
        ...
        console.log(this.checkDimens[checkDimensLength - 1].opinionItemList[opinionItemListLength - 1]);
        if (this.checkDimens[checkDimensLength - 1].opinionItemList[opinionItemListLength - 1].state && this.remark.length === 0) {
