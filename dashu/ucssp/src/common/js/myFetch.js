...
let readAsTextFn = (response) => {
  return new Promise((resolve, reject) => {
    ...
    fileReader.readAsText(response.data); // 开始读取指定的Blob中内容，result属性中，将包含一个字符串以表示所读取的文件内容
...
let urlList = []; // 防止重复提交
const passUrl = ['code/get/dictionarylist', 'code/get/cascader/dictionarylist', 'organization/specialInfo', 'ftb/v1/newfile', '/ftb/v1/id-to-url', '/api/getCascadeCode', '/pfcs/v1/source/get/parts']; // 不需要拦截的url
const MyFetch = {};
// MyFetch.host = host;
...
const doFetch = (url, init) => new Promise((resolve, reject) => {
  ...
  let timeout = 0;
  // if (location.href.indexOf("mloan") > 0) { // 生产环境死等
  //   timeout = 0;
  // } else {
  timeout = init.responseType && init.responseType.timeout ? init.responseType.timeout : 30000;
  // }
  let headers = {
  ...
  // 判断是否是下载柜面面签视频
  if (url.indexOf('/video/download') > -1 || url.indexOf('/appWeb/userManagerRef/export') > -1 || url.indexOf("/appWeb/creditInvestRef/export") > -1 || url.indexOf("/downloadtemplate") > -1 || url.indexOf("/downloaderrorfile") > -1 || url.indexOf('/caes/webapi/impairmentFeeTypeMap/export') > -1 || url.indexOf('interactive/export/price') > -1 || url.indexOf('product/export/zipProduct') > -1) {
  ...
  axios(params).then(async (response) => {
    ...
    // 云屋音视频接口特殊处理
    if (response.config.url.includes("QueueAssignCall")) {
      console.log("========调用云屋接口，需要特殊处理=======");
      resolve(response.data);
      return;
    }

    let isVideo = url.indexOf('exportRepaymentTable') > -1 || url.indexOf("/account/exportBusinessInfo") > -1 || url.indexOf("/account/exportPutoutFile") > -1 || url.indexOf('/video/download') > -1 || url.indexOf('/appWeb/userManagerRef/export') > -1 || url.indexOf("/appWeb/creditInvestRef/export") > -1 || url.indexOf("/downloadtemplate") > -1 || url.indexOf("/downloaderrorfile") > -1 || (url.indexOf("/reports") > -1 && url.indexOf("/export") > -1) || url.indexOf("/productContract/download") > -1 || url.indexOf("/caes/webapi/impairmentFeeTypeMap/export") > -1 || url.indexOf("/branch/template") > -1 || url.indexOf("interactive/export/price") > -1 || url.indexOf('product/export/zipProduct') > -1 || url.indexOf("de-monitor/api/export") > -1 || url.indexOf("v1/download") > -1;
    // 判断是否是下载柜面面签视频
    if (isVideo) {
    ...
    if (response.data.code === "200" || response.data.code === 200) {
      ...
      if (response.data.code === "500" || response.data.code === "9999") {
