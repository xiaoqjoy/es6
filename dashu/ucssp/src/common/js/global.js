...
/**
 * https://zhuanlan.zhihu.com/p/52762903
 */
const CN = [
  {base: 0, cn: '零'},
  {base: 1, cn: '一'},
...
let global = {
  install(Vue, options) {
    ...
    /**
     *
     * @param {String} code 树结构的最后一层
     * @param {Object<Array>} nodeList 树结构
     * @returns {String<Array>} 底层code对应的节点路径
     */
    Vue.prototype.$mathPath = (code, nodeList, key = 'code') => {
      let path = [];
      const mathFn = (code, node, key) => {
        path.push(node[key]);
        if (code === node[key]) {
        ...
        if (node.children && node.children.length) {
          for (let i = 0; i < node.children.length; i++) {
            mathFn(code, node.children[i], key);
          }
        }
        path.pop();
      };
    ...
    // 查询贷款申请征信报告编码
    Vue.prototype.$getCreditId = (applicationId, creditVersion) => {
      let url = '';
      if (creditVersion) {
        url = `${baseApi.getCreditId}${applicationId}/${creditVersion}`;
      } else {
        url = `${baseApi.getCreditId}${applicationId}/id`;
      }
      return new Promise((resolve, reject) => {
        MyFetch
          .get(url)
    ...
    /**
     * @param {string} pageName 柜面面签:counterF2F,远程面签:remoteF2F,审批审计:apraud,调查：survey
     * @param {string} applicationId 单号
     * @param {object} obj {creditReportId:报告编号}
     * @returns {Promise}
     */
    Vue.prototype.$getCreditReport = (pageName, applicationId, obj) => {
      return new Promise(async (resolve, reject) => {
        ...
        if (obj && obj.creditReportId) {
          ...
          if (obj && obj.creditVersion) {
            creditReportId = await Vue.prototype.$getCreditId(applicationId, obj.creditVersion);
          } else {
    ...
    Vue.prototype.$upload = (file, date) => {
      return new Promise((resolve, reject) => {
        let uploadModel = {
          name: file.name,
        ...
        MyFetch.post(baseApi.newFile, {
          storageTime: date || "30d",
          ...
          cos.putObject({
            ...
            Region: "ap-guangzhou",
            Key: res.fileInfo.filePath,
            StorageClass: 'STANDARD',
            Body: file
          }, function(err, data) {
            if (err) {
              Vue.prototype.confirmFn(err, "error");
              return;
            }
            if (data.statusCode === 200) {
              Vue.prototype.confirmFn("上传成功", "success");
