<template>
	<div class="fileUpload">
		...
		<form id="upload" enctype="multipart/form-data" method="post">
			<input style="width: 200px;" :disabled="data.inputIsReadonly==='1'||disabledflag" @change="getFile($event)" type="file" name="file" id="pic" placeholder="导入" />{{parameterValue}}
			...
			<el-button :disabled="data.inputIsReadonly==='1'||disabledflag" type="primary" @click='buttonF()' v-if='buttonFalg'>{{data.addButton}}</el-button>
			<div class="fileTpl">
				或&nbsp;
				<el-input :disabled="data.inputIsReadonly==='1'||disabledflag" v-model="templateId" placeholder="请输入模板编号"></el-input>
				<el-button :disabled="data.inputIsReadonly==='1'||disabledflag" type="primary" @click="changeFileId">确认引用</el-button>
			</div>
		</form>
	</div>
</template>
		...
		methods: {
			...
			getFile(event) {
				...
				if (this.file) {
					...
					let accept = ["excel", "word", "pdf", "doc", "docx", "xlsx", "xls", "xml"];
...
//				}
			...
			changeFileId() {
				this.$MyFetch.post(`${poductApi.checkTemplate}?templateNo=${this.templateId}`).then((data) => {
					if (data) {
						this.parameterValue = this.templateId;
						this.templateId = "";
						this.changeData();
						this.$message.success("引用成功");
					} else {
						this.$message.error("引用失败");
					}
				}).catch((err) => {
					this.$error(err.message);
				});
