			...
			<div class="lookPopup_main">
				...
				<div class="popup_paddindiv">
					...
					<div class="popup_button">
						...
						<button @click="exportProductData">导出产品数据</button>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	export default{
		...
		methods: {
			...
			exportProduct() {
				if (this.radio >= 0) {
					let product = this.publishList[this.radio];
					this.$MyFetch.get(poductApi.exportProduct, {productId: product.productId, version: product.version})
						.then((data = {}) => {
							console.log(data);
			...
			exportProductData() {
				if (this.radio >= 0) {
					let product = this.publishList[this.radio];
					this.$MyFetch.get(poductApi.zipProduct, {productId: product.productId, version: product.version})
						.then((data = {}) => {
							this.$download(data);
						})
						.catch((err) => {
					...
					this.$MyFetch.post(poductApi.getProductList, json)
						.then((data = {}) => {
							if (data.list && data.list.length > 0) {
								this.confirmFn("此产品存在待配置产品，请至产品配置页面进行操作");
							} else {
								this.$router.push({path: 'productConfigure', query: {productId: productId, version: version, toPath: "productList", disabledflag: disabledflag}});
							}
						})
