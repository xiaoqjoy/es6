<template>
    <div class="add-user">
      ...
      <div class="dialog-box">
        ...
        <div class="dialog-box__middle">
          <el-form :inline="true"
            ...
            <el-form-item prop="userId" class="form__item" label="用户编号">
              <el-input
                @change="getUserInfo(editObj.userId)"
                v-model="editObj.userId"
                placeholder="请输入um账号查询"
        ...
        <div class="dialog-box__bottom bg__white" style="height:20px"></div>
      </div>
    </div>
</template>
  ...
  export default{
    ...
    data() {
      return {
        ...
        rules: {
          userName: [
            { required: true, message: '请输入um账号获取用户名', trigger: 'change' }
          ],
          mobilePhoneNum: [
      ...
      submitFn() {
        this.$refs.checkForm.validate((valid) => {
          if (valid) {
            ...
            if (this.editObj.type === 'add') {
              this.$MyFetch.post(api.user.add, this.editObj)
                .then(() => {
              ...
              this.$MyFetch.put(api.user.edit, this.editObj)
                .then(() => {
