    ...
    <div class="detection-msg">
      ...
      <div class="dialog-box">
        ...
        <div class="dialog-box__bottom">
          ...
          <!-- 726新增匹配方式：身份证号码 -->
          <div v-if="certificateNumAttention.length > 0">
            <div class="bottom-content" v-for="(item, index) in certificateNumAttention" :key="index">
              <div class="clearfix bottom-content_title">
                <div class="content__title">
                  <span class="font_gray">匹配方式:</span>
                  <span v-if="certificateNumAttention.length > 0">{{certificateNumAttention[index].matchModeCode}}</span>
                </div>
                <div class="content__title">
                  <span class="font_gray">匹配信息:</span>
                  <span v-if="certificateNumAttention.length > 0">{{certificateNumAttention[index].matchInfoTxt}}</span>
                </div>
                <div class="content__title">
                  <span class="font_gray">匹配人:</span>
                  <span
                    v-if="certificateNumAttention.length > 0"
                  >{{certificateNumAttention[index].matchContactTxt}}</span>
                </div>
                <div class="content__title">
                  <span class="font_gray">与借款人的关系:</span>
                  <span v-if="certificateNumAttention.length > 0">{{certificateNumAttention[index].relationshipCode}}</span>
                </div>
              </div>
              <div class="eltables">
                <el-table :data="item.attentionMatchArray" style="width:100%">
...
export default {
  ...
  created() {
    console.log(this.appOrAud, this.disabled);
  ...
  methods: {
    goHistory(id, certificateId, type) {
      if (type === "贷款审计") {
        let routeData = this.$router.resolve({
          ...
          query: { appID: id, isEdit: true }
        });
        window.open(routeData.href, "_blank");
      } else if (type === "贷款审批" || type === "待客户确认") {
        let routeData = this.$router.resolve({
    ...
    //  this.url = aprAudApi.attentionInfo.replace("{applicationId}", this.applicationId);
      this.$MyFetch.get(aprAudApi.attentionInfo.replace("{applicationId}", this.applicationId), data).then((data = {}) => {
        ...
        console.log(data.certificateNumAttention);
        if (data.certificateNumAttention.length > 0) {
          this.certificateNumAttention = data.certificateNumAttention;
          this.certificateNumAttentionList =
            data.certificateNumAttention[0].attentionMatchArray;
          this.certificateNumAttentionList.splice(20, 100);
          console.log(this.certificateNumAttention, this.certificateNumAttentionList);
        }
