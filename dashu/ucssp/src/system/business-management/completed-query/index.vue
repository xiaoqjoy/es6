  ...
  <div>
    <div class="content">
      ...
      <div class="b_content">
        ...
        <el-select clearable class="select_72" v-model="orgId"  placeholder="合作方名称">
          <el-option v-for="item in list_partner"
            :key="item.codeValue"
            :label="item.codeText"
            :value="item.codeValue">
          </el-option>
        </el-select>
        <el-select clearable class="select_72" v-model="fundingPartyId"  placeholder="资金方名称">
          <el-option v-for="item in list_capital"
            :key="item.codeValue"
            :label="item.codeText"
            :value="item.codeValue">
          </el-option>
...
export default {
  ...
  methods: {
    ...
    getPartner() {
      this.$MyFetch.get(configApi.partner)
        .then(_data => {
          this.list_partner = _data;
        }).catch(_err => {
          this.$error(_err.message);
        });
    },
    getFund() {
      this.$MyFetch.get(configApi.found)
        .then(_data => {
          this.list_capital = _data;
        }).catch(_err => {
          this.$error(_err.message);
        });
    },
    ...
    refer(pageNum) {
      if (
        ...
        if (!isNaN(this.amount)) {
          ...
          orgId: this.orgId,
          fundingPartyId: this.fundingPartyId
        };
        this.loading = true;
        console.log(data);
