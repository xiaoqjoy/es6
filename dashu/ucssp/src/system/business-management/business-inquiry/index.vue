  ...
  <div>
    <div class="content">
      ...
      <div class="b_content">
        ...
        <el-select clearable class="select_72" v-model="surveyStatus" placeholder="调查状态">
          <el-option
            v-for="item in surveyStatusList"
            :key="item.itemCode"
            :label="item.itemDesc"
            :value="item.itemCode"
          ></el-option>
        </el-select>
        <el-select clearable class="select_72" v-model="patchStatus" placeholder="补件状态">
          <el-option
            v-for="item in patchStatusList"
            :key="item.itemCode"
            :label="item.itemDesc"
            :value="item.itemCode"
          ></el-option>
      ...
      <div class="tables">
        ...
        >
          ...
          <el-table-column prop="surveyStatus" label="调查状态"></el-table-column>
          <el-table-column prop="patchStatus" label="补件状态"></el-table-column>
          ...
          <el-table-column prop="riskLevel1" label="业务评分1：" width="120"></el-table-column>
          ...
          <el-table-column prop="riskLevel2" label="业务评分2：" width="120"></el-table-column>
          <el-table-column prop="switchHighScoreInd" label="是否超分贷">
            <template slot-scope="scope">
              <span>{{scope.row.switchHighScoreInd ? '是' : '否'}}</span>
            </template>
          </el-table-column>
          <!-- 渤税需求0116版本延期至0213版本，暂时注释 -->
          <el-table-column
            prop="switchProductInd"
            label="是否为转产品" width="120">
            <template slot-scope="scope">
              <span>{{scope.row.switchProductInd ? '是' : '否'}}</span>
            </template>
          </el-table-column>
          <el-table-column
            prop="switchProductDesc"
            label="转一级产品名称" width="120">
          </el-table-column>
          ...
          <el-table-column prop="operation" fixed="right" width="250" label="操作">
            <template slot-scope="scope">
              ...
              <router-link
                v-if="scope.row.taskNode === 'approveTask'? true:false"
                target="_blank"
                :to="{name:'approval-pending-start', query:{appID:scope.row.applicationId, comeFrom: 'noPending', isEdit:true}}"
              >
                <el-button type="text" size="small">详情</el-button>
              </router-link>
              <router-link
                v-if="scope.row.taskNode === 'auditTask'? true:false"
                target="_blank"
                :to="{name:'audit-pending-start', query:{appID:scope.row.applicationId, comeFrom: 'noPending', isEdit:true}}"
              >
                <el-button type="text" size="small">详情</el-button>
              </router-link>
              <router-link
                v-if="scope.row.taskNode === 'preAuditTask'? true:false"
                target="_blank"
                :to="{name:'pre-audit', query:{appID:scope.row.applicationId, isEdit:true}}"
              >
                <el-button type="text" size="small">详情</el-button>
              </router-link>
              <router-link
                v-if="scope.row.taskNode === 'preApproveTask'? true:false"
                target="_blank"
                :to="{name:'pre-approval', query:{appID:scope.row.applicationId, isEdit:true}}"
              >
                <el-button type="text" size="small">详情</el-button>
              </router-link>
            </template>
...
export default {
  ...
  methods: {
    ...
    getMultiSelect() {
      let data = {
        categoryCodes: ['survey_status_show', 'patch_status_show'],
        getAll: true
      };
      this.$MyFetch.get(configApi.multipleSelect, data)
        .then(_data => {
          this.patchStatusList = _data.patch_status_show;
          this.surveyStatusList = _data.survey_status_show;
        }).catch(_err => {
