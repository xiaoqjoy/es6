<template>
  <div class="import-message">
        ...
        <div class="dialog-box__content">
          <div class="content">
            ...
            <div class="content__title clearfix">
              ...
              <div class="f__left">
                <span class="font-gray">业务来源:</span>
                <span class="font-black">{{patchMsg.businessSourceName}}</span>
              </div>
              <div class="f__left">
                <span class="font-gray">单位名称:</span>
                <span class="font-black">{{patchMsg.workplaceName}}</span>
              </div>
              <div class="f__left">
                        ...
                        <li class="table__body" v-for="(item, index) in coownerList" :key="index">
                          <div class="table__tr">
                            ...
                            <ul class="table__tr__top-list" v-show="!item.showEle">
                              ...
                              <li class="table__td">{{changeEstate(item.estateNum)}}</li>
    ...
    <printer-pop v-if="showPrinterPop" @close="showPrinterPop = false" />
  </div>
</template>
...
export default {
  props: [ 'isReadonly', 'isDispose', 'siTaskId', 'siBusinessId', 'applicationId', 'refresh', 'item' ],
  data() {
    return {
      ...
      coownerRule: {
        ...
        certificateNumInd04: [
          ...
          { validator: myRules.otherCardRule, trigger: "blur" }
          // { validator: myRules.officialCardRule, trigger: "blur" }
        ],
        certificateNumInd06: [
          {
            required: true,
  ...
  methods: {
    ...
    changeEstate(num) {
      for (let i = 0; i < this.estate.length; i++) {
        if (num === this.estate[i].estateNum) {
          return this.estate[i].addressDesc;
        }
      }
    },
      // 获取下拉菜单
  ...
  created() {
    ...
    this.getLibraryApi('certificate_type_code')
          .then(data => {
            ...
            console.log(this.certificateLibrary, 'dadada');
            data.forEach((v) => {
...
.import-message /deep/ .button__yellow:disabled {
