<template>
  <div>
            ...
            <el-table
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <el-button type="text" size="small" @click="toPatchDetails(scope.$index, scope.row)">补件处理
                  </el-button>
                </template>
              </el-table-column>
            </el-table>
      ...
      <patchDetails v-if="alertPatch && !siCategoryCd" @clsoe_windowAlert="close_patchDetails" :isReadonly="isReadonly" :refresh="getTable"
       :isDispose="isDispose" :siTaskId="siTaskId" :siBusinessId="siBusinessId"
      ...
      <easy-loan-path-detail v-if="alertPatch && siCategoryCd" @clsoe_windowAlert="close_patchDetails" :isReadonly="isReadonly" :refresh="getTable"
       :isDispose="isDispose" :siTaskId="siTaskId" :siBusinessId="siBusinessId" :disabled="false" :isDisabled="false" :showRemarkForm="true" :showPending="true"
...
import patchDetails from "./patchDetails/index";
import easyLoanPathDetail from './patchDetails/easy_loan_path_detail.vue';
...
export default {
  ...
  methods: {
    ...
    getTable(page, pageSize) {
      ...
      this.$MyFetch.get(patchApi.dispose.newTasks, data)
      .then((data) => {
        if (data) {
