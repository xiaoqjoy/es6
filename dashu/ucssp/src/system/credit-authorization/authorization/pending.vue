	...
	export default {
		data() {
			return {
				...
				columnList: [
					...
					{ label: '客户姓名', prop: 'custName' },
					{ label: '身份证号码', prop: 'certificateNum' },
					{ label: '产品一', prop: 'productName1' },
					{ label: '产品二', prop: 'productName2' },
					{ label: '个贷服务中心', prop: 'loanCenter' },
					{ label: '备签提交时间', prop: 'sloveTime' }
				]
			};
		},
		methods: {
        ...
        const nameRule = /^[\u4e00-\u9fa5·]+$/;
				if (!this.addJson.certificateNum || !this.addJson.custName || !this.addJson.reportSourceCode) {
					this.$error("下列信息必填");
				} else if (!nameRule.test(this.addJson.custName) && this.addJson.custName) {
          this.$error('客户姓名存在非法字符！');
        } else {
