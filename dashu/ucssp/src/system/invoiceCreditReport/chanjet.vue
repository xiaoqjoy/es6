...
export default {
  ...
  computed: {
    ...
    saleRegionListToYear() {
      let saleRegionList = this.saleRegionList.filter((item) => {
        return (item.month.substring(0, 4) - 0) === (this.relDate.getFullYear() - 0);
      });
    ...
    saleRegionListLastYear() {
      let saleRegionList = this.saleRegionList.filter((item) => {
        return (item.month.substring(0, 4) - 0) === (this.relDate.getFullYear() - 1);
      });
      return this.saleRegionFilter(saleRegionList);
    }
  },
  methods: {
    ...
    nearMonthCount(num, key) {
      ...
      this.salesInvoiceInfo.forEach((item) => {
        let last = this.$Moment(item.month.substring(0, 4) + "-" + item.month.substring(4, 6));
  ...
  created() {
    ...
    if (applicationId) {
      this.$MyFetch.post(`${api.channel}/CHANJET/invoice/${applicationId}`)
      .then((data) => {
