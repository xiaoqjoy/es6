      ...
      <div class="complete-product">
        <div class="dialog-box__middle-form">
          ...
          <div class="form__bottom">
            <el-button class="add finish width140" :disabled="isDisabled"  type="primary " @click="finish">确认</el-button>
          </div>
        </div>
      </div>
    </div>
    <el-dialog :title="importTitle"
  ...
  export default {
    ...
    methods: {
      ...
      importExcel(_data) {
        ...
        this.$MyFetch.post(loanApi.importFromExcel, _data, {timeout: 60000}).then(_data => {
          loading.close();
          this.importTitle = _data.bizCode === '200' ? '导入成功' : '导入失败';
          this.isSuccess = _data.bizCode === '200';
          if (this.isSuccess) {
            this.successCount = _data.successCount;
          }
          if (!this.isSuccess) {
            this.failureCount = _data.errorInfo.length;
            let failureItems = _data.errorInfo.map(_item => {
              return _item.lineNumber;
            });
