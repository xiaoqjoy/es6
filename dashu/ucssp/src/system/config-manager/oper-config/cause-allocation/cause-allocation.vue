<template>
  <div>
    ...
    <addCauseAllocation :cause_x="cause" :systemNum="systemNum" :for="for_" v-if="alert_" @clsoe_windowAlert='close_1' />
    <editCauseAllocation :cause_x="cause" :current_list='currentList' :list_="list" v-if="alert_e" @clsoe_windowAlert_e='close_1_e' />
    ...
    <whetherSaveX :cause_x="cause" :msg="msg" v-if="toSaveD" @clsoe_sf='console_s' />
  </div>
</template>

<script type="text/ecmascript-6">
import pagination from "@components/pagination"; // 分页组件
import addCauseAllocation from "../cause-allocation/addCauseAllocation/"; // 新增调查原因配置组件
import editCauseAllocation from "../cause-allocation/editCauseAllocation/"; // 编辑调查原因配置组件
...
import whetherSaveX from "../cause-allocation/whetherSaveX/index"; // 是否保存已经修改状态组件
...
export default {
  data() {
    return {
      ...
      alert_: false, // 新增调查原因配置组件
      alert_e: false, // 编辑调查原因配置组件
      isSelect: false, // 是否选择表格数据
      invalid: false, // 停用
      used: false, // 启用
      toSaveD: false, // 是否保存默认不显示
  ...
  // computed: {
  //   toSaveD() {
  //     return this.$store.getters.toSaveD;
  //   }
  // },
  methods: {
    // 关闭是否保存组件
    ...
    reexamine(checkBy, checkByPwd) {
      ...
      }/${"investigation_reason"}/checkAndSave`;
      this.$MyFetch
        .post(url, data)
        .then((data = {}) => {
          console.log(data);
    ...
    save() {
      ...
      }/${"investigation_reason"}/checkModify`;
      this.$MyFetch
        .get(url)
        .then((data = {}) => {
          console.log(data);
    ...
    cause(pageNum) {
      ...
      }/${"investigation_reason"}/levelOne/list`;
      let data = {
        pageNum: pageNum, // 当前页
    ...
    deadText() {
      ...
      }/${"investigation_reason"}/changeStatus`;
      this.$MyFetch
        .post(url, data)
        .then((data = {}) => {
          console.log(data);
    ...
    add_alert() {
      ...
      }/${"investigation_reason"}/newLevelOneRecordNum`;
      this.$MyFetch
        .get(url)
        .then((data = {}) => {
          console.log(data);
