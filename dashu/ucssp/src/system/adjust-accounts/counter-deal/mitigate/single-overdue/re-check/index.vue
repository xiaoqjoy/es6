...
export default {
    ...
    methods: {
        // 查询总单接口
        queryPayInteFineAdjustList(res = {"phaseNoList": ["0020"], "flowNo": "reductionFlow", "transType": 15}, current = 1) {
        ...
        // 记账
        webapiAccounting(res) {
            return new Promise((resolve, reject) => {
                this.$MyFetch.post(counterDealApi.webapiAccounting, res)
                .then((data = {}) => {
                    resolve(data);
                })
                .catch((err) => {
        ...
        submitReconfirmBackFun() {
             let queryData = {
                "flowNo": "reductionFlow",
                "transSerialNos": this.transSerialNoList
            };
            this.webapiAccounting(queryData)
            .then((data) => {
