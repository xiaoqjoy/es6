<template>
  <div>
    <div class="content">
      ...
      <!-- 底部 -->
      <div class="enable">
        <el-button class="t_button dead_t" @click="openReturn">退回</el-button>
        <el-button class="t_button initiate_t" @click="openComponent">提交</el-button>
      </div>
    </div>
    <editFirstPunishment
    ...
    <returningSubmission
      ...
      v-if="accordingReturn"
      @close_return="close_submission"
    />
  </div>
</template>
...
import editFirstPunishment from "./editFirstPunishment/index"; // 编辑组件
import returningSubmission from "./returningSubmission/index"; // 引入退回和提交组件
...
export default {
  data() {
    return {
      ...
      message: "", // 退回和提交组件的头部的提示
      showIf: false, // 编辑组件默认不显示
      accordingReturn: false, // 退回和提交组件默认不显示
      editDetails: {}, // 减免编辑详情页
      selectedRow: [], // 选中复选框
      pageConfig: {
        account: 0, // 总条数
  ...
  methods: {
    ...
    returnInterface() {
      ...
      let url = checkingApi.transPayInfoh;
      let data = {
    ...
    // 打开退回组件
    openReturn() {
      this.message = "是否确定退回选中的记录";
      if (this.selectedRow.length > 0) {
        this.accordingReturn = true;
      } else {
    ...
    submitInterface() {
      ...
      let url = checkingApi.transPayInfoi;
      let data = {
        transSerialNos: selectedData,
        flowNo: "freePayBuyBackFlow"
      };
    ...
    compensatoryRelief(current) {
      ...
      current = current || 1;
      let url = checkingApi.transPayInfob + "?current=" + current + "&size=10";
      this.$MyFetch
        .post(url, res)
        .then((data = {}) => {
    ...
    edit(detail) {
      ...
      this.$MyFetch
        ...
        .then((data = {}) => {
          ...
          this.editDetails.loanNo = detail.loanNo;
          this.editDetails.relateId = detail.relateId;
          this.editDetails.accDate = this.getNowFormatDate();
          this.showIf = true;
        })
        .catch(err => {
          this.$error(err.message);
  ...
  components: {
    pagination, // 分页组件
    editFirstPunishment, // 编辑组件
    returningSubmission // 退回和提交组件
  },
  created() {
    this.compensatoryRelief();
  }
