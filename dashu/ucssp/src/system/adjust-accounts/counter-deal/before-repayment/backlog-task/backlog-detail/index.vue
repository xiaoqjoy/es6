    ...
    <div class="dialog-box" v-loading="boxLoading">
                ...
                <div class="tableBodyTr">
                  ...
                  <div class="tableBodyTd2">
                    <el-form-item prop="payInte">
                      <el-input v-model="writeInfo.payInte" :disabled="!remotePrepay"></el-input>
                    </el-form-item>
                  </div>
                  <div class="tableBodyTd3">
                ...
                <div class="tableBodyTr">
                  ...
                  <div class="tableBodyTd2">
                    <el-form-item prop="ddPayCorp">
                      <el-input v-model="writeInfo.ddPayCorp" :disabled="!remotePrepay"></el-input>
                    </el-form-item>
                  </div>
                  <div class="tableBodyTd3">
                ...
                <div class="tableBodyTr">
                  ...
                  <div class="tableBodyTd2">
                    <el-form-item prop="ddPayInte">
                      <el-input v-model="writeInfo.ddPayInte" :disabled="!remotePrepay"></el-input>
                    </el-form-item>
                  </div>
                  <div class="tableBodyTd3">
                ...
                <div class="tableBodyTr">
                  ...
                  <div class="tableBodyTd2">
                    <el-form-item prop="ddPayFine">
                      <el-input v-model="writeInfo.ddPayFine" :disabled="!remotePrepay"></el-input>
                    </el-form-item>
                  </div>
                  <div class="tableBodyTd3">
                ...
                <div class="tableBodyTr">
                  ...
                  <div class="tableBodyTd2">
                    <el-form-item prop="ddPayCompound">
                      <el-input v-model="writeInfo.ddPayCompound" :disabled="!remotePrepay"></el-input>
                    </el-form-item>
                  </div>
                  <div class="tableBodyTd3">
                ...
                >
                  ...
                  <div class="tableBodyTd2">
                    <el-form-item :prop="'feeMap.'+feeIndex+'.feeAmount'" :rules="ruless.feeAmount">
                      <el-input v-model="feeItem.feeAmount" :disabled="!remotePrepay"></el-input>
                    </el-form-item>
                  </div>
                  ...
                  <div class="tableBodyTd3">
                    ...
                    >
                      <el-input v-model="feeItem.actualFeeAmount" :disabled="feeItem.feeType === '3010'"></el-input>
                    </el-form-item>
                  </div>
                  <div class="tableBodyTd4">
                    <el-form-item
                    ...
                    >
                      <el-input v-model="feeItem.reduceFeeAmount" :disabled="feeItem.feeType === '3010'"></el-input>
                    </el-form-item>
                  </div>
      ...
      <div class="dialog-box__footer">
        <div class="footerBtn">
          ...
          <el-button v-if="showSaveBtn" class="t_button affirm_t button" @click="saveHandler">保存</el-button>
          <el-button class="t_button affirm_t button" @click="affirm_fun" :disabled="status !== '已完成'">确认</el-button>
        </div>
      </div>
      <!-- 提示窗 == 发送邮件 -->
...
import { counterDealApi, prepayment } from "@/system/adjust-accounts/counter-deal/js/server"; // 接口
...
export default {
  ...
  created() {
    ...
    let para = {
      transSerialNo: this.trans_serial_no,
  ...
  methods: {
    saveHandler() {
      if (!this.validateFn('ChannelObj') || !this.validateFn('writeInfo') || !this.validateFn('feeMapObj')) {
        return;
      };
      let checkQueryData = {
        loanNo: this.loan_no,
        channel: this.ChannelObj.ChannelValue // 交易渠道
      };
      let uploanData = {
        applyId: this.apply_no,
        productId: this.productId
      };
      let prepaymentCheckAmtData = this.countQueryData();
      let promiseList = [
        this.prepaymentCheckChannel(checkQueryData),
        this.prepaymentCheckAmt(prepaymentCheckAmtData),
        this.prepaymentCheckUploan(uploanData)
      ];
      let feeInfoList = this.composeData(this.writeInfo);
      let data = {
        feeInfoList,
        billNo: this.trans_serial_no,
        channelCode: this.ChannelObj.ChannelValue
      };
    ...
    saveApi(_data) {
      this.$MyFetch.post(prepayment.saveDetail, _data).then((_data) => {
        if (_data.result) {
          this.$confirm('请确认客户是否可进入资料签署环节', {type: 'warning'}).then(() => {
            this.subStatusFlow();
          });
        } else {
          this.confirmFn('保存失败', 'error');
        }
      }).catch(_err => {
    ...
    subStatusFlow() {
      ...
      this.$MyFetch.post(prepayment.subStatusFlow, data).then(_data => {

      }).catch(_err => {
        this.$error(_err.message);
      });
    },
    composeData(writeInfo) {
      let feeInfoList = [];
      let feeNameList = ['本金', '利息', '应还未结清本金', '应还未结清利息', '应还未结清罚息', '应还未结清复利'];
      let feeTypeList = ['CORP', 'INTE', null, null, null, null];
      let payAmountList = ['payCorp', 'payInte', 'ddPayCorp', 'ddPayInte', 'ddPayFine', 'ddPayCompound'];
      feeNameList.forEach((_v, _i) => {
        let obj = {
          feeName: _v,
          feeType: feeTypeList[_i],
          payAmount: writeInfo[payAmountList[_i]],
          actualAmount: writeInfo[`actual_${payAmountList[_i]}`],
          reductAmount: writeInfo[`reduce_${payAmountList[_i]}`]
        };
        feeInfoList.push(obj);
      });
      this.feeMapObj.feeMap.forEach(_v => {
        let obj = {
          feeName: _v.feeName,
          feeType: _v.feeType,
          payAmount: _v.feeAmount,
          actualAmount: _v.actualFeeAmount,
          reductAmount: _v.reduceFeeAmount
        };
        feeInfoList.push(obj);
      });
      return feeInfoList;
    ...
    pendingPrepay(para = {}) {
      return new Promise((resolve, reject) => {
        let url = counterDealApi.pendingPrepay;
        // if (this.remotePrepay) {
        //   url = prepayment.queryDetailInfoByTrial;
        // }
        this.$MyFetch
          .post(url, para)
          .then((data = {}) => {
    ...
    getInfo(data) {
      ...
      if (this.remotePrepay) {
        this.writeInfo = Object.assign({}, this.writeInfo, {
          actual_payCorp: data.actualCorp, // 实还剩余本金
          actual_payInte: data.actualInte, // 实还应还利息
          actual_ddPayCorp: data.ddActualCorp || 0, // 实还未结清本金
          actual_ddPayInte: data.ddActualInte || 0, // 实还当前利息
          actual_ddPayFine: data.ddActualFine || 0, // 实还未结清罚息
          actual_ddPayCompound: data.ddActualCompound || 0, // 实还未结清复利
          reduce_payCorp: data.reduceCorp || 0, // 减免剩余本金
          reduce_payInte: data.reduceInte || 0, // 减免应还利息
          reduce_ddPayCorp: data.ddReduceCorp || 0, // 减免未结清本金
          reduce_ddPayInte: data.ddReduceInte || 0, // 减免当前利息
          reduce_ddPayFine: data.ddReduceFine || 0, // 减免未结清罚息
          reduce_ddPayCompound: data.ddReduceCompound || 0 // 减免未结清复利
        });
      }
      // 动态费用列表，为了验证
      if (data.feeMap.length > 0) {
        ...
        data.feeMap.forEach(element => {
          ...
          if (this.remotePrepay) {
            obj.actualFeeAmount = element.actualAmount || 0;
            obj.reduceFeeAmount = element.reduceAmount || 0;
          }
          feeMap.push(obj);
        });
    ...
    clearValidate() {
      this.$nextTick(() => {
        this.$refs.ChannelObj.clearValidate();
      });
    },
    // 关闭弹窗
    close() {
