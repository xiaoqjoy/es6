<template>
  <div class="content">
      ...
      <div class="table_box">
        ...
        >
          ...
          <el-table-column width="140px" fixed="right" label="操作">
            <template slot-scope="scope">
              <el-button type="text" @click="sendback_fun(scope.row)">退回</el-button>
    ...
    <div class="delete_submit">
      <div>
        <el-button class="t_button initiate_t button" @click="keepaccounts_fun">记账</el-button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    sendback_fun(row) {
      this.$confirm("是否确认回退？", "提示", {
        confirmButtonText: "是",
        cancelButtonText: "否",
        center: true
      })
        .then(() => {
          this.confirm_sendback_fun(row.transSerialNo);
        })
        .catch(() => {
          this.$message({
            type: "info",
    ...
    confirm_sendback_fun(transSerialNo) {
      let params = {
        flowNo: this.searchData.flowNo,
        phaseNo: this.searchData.phaseNo,
        flowAction: "BACK",
        flowObjectList: [transSerialNo]
      };
      this.$MyFetch
    ...
    keepaccounts_fun() {
      if (!this.transSerialNoList[0]) {
        ...
        this.$MyFetch
          .post(prepaymentBreak.bookBatch, {
            flowNo: "RepaymentBreakFlow",
