<template>
  <div class="content">
    <div class="sponsor">
      <button class="add button" @click="detail_alert()">+ 新增</button>
      <!-- 查询这一行 -->
  ...
  methods: {
    ...
    handleSelectionChange(data) {
      ...
      data.forEach(item => {
        ...
        this.transSerialNoList.push(item.transSerialNo);
      });
      console.log("handleSelectionChange", this.transSerialNoList);
    },
    ...
    refer(current) {
      ...
      this.searchData.queryDateBeg = this.applicationDate
        ? this.applicationDate[0]
        : "";
      this.searchData.queryDateEnd = this.applicationDate
        ? this.applicationDate[1]
        : "";
      this.referList(current, this.searchData);
    },
    ...
    confirm_delete_fun() {
      let params = {
        flowNo: this.searchData.flowNo,
        phaseNo: this.searchData.phaseNo,
        flowAction: "DEL",
        flowObjectList: this.transSerialNoList
      };
      this.$MyFetch
        .post(prepaymentBreak.processFlow, params)
        .then((data = {}) => {
          this.delete_hint = false;
    ...
    detail_alert() {
      this.detailData.type = "add";
      this.alert_detail = true;
    },
    detail_save(loanNo) {
      this.lbLoanNo = loanNo;
      this.searchData.eq.lbLoanNo = loanNo;
      this.refer(this.pageConfig.currentPage);
      this.alert_detail = false;
    }
  },
