<template>
    <div class="content">
      ...
      <div class="sponsor">
        ...
        <!-- 新增单笔还款  -->
         <addPriorMatch v-if="alert_" @clsoe_windowAlert='close_1' @prepaymentList='prepaymentList("", pageConfig.currentPage)'></addPriorMatch>
        <!-- 详情 -->
        <sponsorDetail v-if="alert_detail" @clsoe_windowDetail='close_2' :tetailData = "tetail_data" @reload="reload"></sponsorDetail>
        <!--提交 再次确认组件 -->
...
export default {
  ...
  methods: {
    ...
    elOchange(file, filelist) {
       if (filelist.length > 1) { // 让上传文件只有一个
    ...
    finish() {
        ...
        this.$refs.writeInfo.validate((valid) => {
            if (valid) {
                ...
                this.hint = false;
                this.fileMsg = "";
                if (this.accept.indexOf(fileType.toLocaleUpperCase()) < 0 && fileType) {
                    this.hint = true;
                ...
                _that.submitPrepayment(queryData)
                .then((data) => {
                    // 邮件附件为空也可保存
    ...
    deletePrepaymentn(res) {
        return new Promise((resolve, reject) => {
            this.$MyFetch.post(counterDealApi.deletePrepaymentn, res)
            .then((data = {}) => {
                resolve(data);
            })
            .catch((err) => {
    ...
    prepaymentOnLineList(res) {
        return new Promise((resolve, reject) => {
            this.$MyFetch.post(counterDealApi.prepaymentOnLineList, res)
            .then((data = {}) => {
                resolve(data);
            })
            .catch((err) => {
            ...
            this.prepaymentOnLineList(prepaymentOnLineData)
             .then((data) => {
                ...
                if (data.loanNo.length > 0) {
                    ...
                    // 提示
                    this.$message.success("有" + data.loanNo.length + "条数据为线下对接机构，请发送邮件通知！");
                } else { // 否则提交数据
                    let queryData = {
                        "phaseNo": '0010',
                        "flowNo": "repaymentFlow",
    ...
    detail_alert(rowData) {
        rowData.transSerialNo = rowData.trans_serial_no;
    ...
    rowCheck(row, column, e) {
      if (!this.submitDeleteData[0]) {
        ...
        this.loanNoList = [row.loan_no];
        this.businessSum = row.business_sum;
      } else if (this.submitDeleteData[0] && row.trans_serial_no === this.submitDeleteData[0]) {
        this.$refs.multipleTable.clearSelection();
        this.submitDeleteData = [];
        this.loanNoList = [];
        this.businessSum = '';
