...
import sponsorDetail from "../re-check/sponsor-detail/"; // 发起-明细
...
export default {
  ...
  methods: {
    ...
    // 单笔还款记账
    batchPrepaymentFileAcct(res) {
        return new Promise((resolve, reject) => {
            this.$MyFetch.post(counterDealApi.batchPrepaymentFileAcct, res)
                .then((data = {}) => {
                    resolve(data);
                })
                .catch((err) => {
    ...
    affirm_submit_fun() {
        this.submit_hint = false;
            ...
            let submitData = {
                flowNo: "batchPrepaymentFlow",
                batchNos: [this.deleteSubmitData.batchNo]
            };
            this.batchPrepaymentFileAcct(submitData)
                .then((data) => {
                    this.reload();
                })
                .catch((err) => {
