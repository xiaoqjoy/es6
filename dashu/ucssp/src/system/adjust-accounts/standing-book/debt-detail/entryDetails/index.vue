...
export default {
  ...
  methods: {
    ...
    loanParameter(res, current) {
      res = res || {};
      current = current || 1;
      let url = checkingApi.LoanSearchaa + "?current=" + current + "&size=10";
      this.$MyFetch
        .post(url, res)
        .then((data = {}) => {
    ...
    // // 查询
    refer() {
      this.searchData.eq.subject_no = this.batchNumberIntroduced;
      this.searchData.eq.sub_subject_no = this.auxiliaryCourse;
      this.searchData.eq.loan_no = this.queryJY;
  ...
  props: ["batchNumberIntroduced", "queryJY", "auxiliaryCourse"],
  components: {
    pagination
  }
};
