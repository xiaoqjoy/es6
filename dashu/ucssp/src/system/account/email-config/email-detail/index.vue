    ...
    <div class="import-message" >
      ...
      <div class="dialog-box" v-loading='formLoading'>
        ...
        <div class="dialog-box__top">
          <h5 class="title bg__white">邮箱详情</h5>
                  ...
                  <li class="f__left">
                    ...
                    <div class="email_list" v-if="bankReceiverAddressesList.length>0">
                      <div v-for="(o, i) in bankReceiverAddressesList" :key="i" class="text item" >
                        ...
                        style="color: #F55F5F;"
                        class="f__right" size="mini">删除</el-button>
                      </div>
                    </div>
                  </li>
                  <li class="f__left">
...
export default {
  ...
  created() {
    ...
    if (this.formData.bankReceiverAddresses) {
      this.bankReceiverAddressesList = this.formData.bankReceiverAddresses.split(";");
    }
    if (this.formData.copyToAddresses) {
      this.copyToAddressesList = this.formData.copyToAddresses.split(";");
    }
    // console.log(this.lineIds, this.linkBranchDss);
  },
  mounted() {
    this.ruleForm = Object.assign({}, this.formData, {lineName: ''});
    this.ruleForm.dealDate = new Date().normalizeTime({ format: "" }).strTime;
    this.ruleForm.dealUserId = userId;
    // console.log(this.formData);
    this.activePartner.partnerName = this.formData.partnerName;
    this.activePartner.partnerCd = this.formData.lineId;
    this.ruleForm.lineName = this.activePartner.partnerName;
    this.ruleForm.lineId = this.activePartner.partnerCd;
    this.activeLinkBranch.organizationId = this.formData.linkBranchDs;
    for (var i = 0; i < this.linkBranch.length; i++) {
      if (
        this.activeLinkBranch.organizationId ===
        this.linkBranch[i].organizationId
      ) {
        this.activeLinkBranch.organizationName = this.linkBranch[i].organizationName;
      }
    }
...
.email-config .wrap .el-input__inner {
  border-color: #d0d0d0;
}
</style>
