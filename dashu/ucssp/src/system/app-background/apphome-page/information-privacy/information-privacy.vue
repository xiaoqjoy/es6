      ...
      this.$MyFetch
        ...
        .then((data = {}) => {
          ...
          for (let i = 0; i < data.length; i++) {
            // 判断获取信息保密说明数据
            if (data[i].bannerType === "confidentiality_statement") {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let data = {
            ...
            bannerContent: this.form.content_text,
            bannerImgUrl: this.imageUrl,
            websiteUrl: this.form.input,
            bannerType: "confidentiality_statement"
          };
          // 调取保存接口
          ...
          this.$MyFetch
            .post(url, data)
            .then((data = {}) => {
              this.confirmFn("信息保密声明新增成功", "success");
              this.refer(); // 保存后刷新页面
            })
            .catch(err => {
              this.$error(err.message);
