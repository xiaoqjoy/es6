<template>
  <div class="table-area">
    ...
    style="width: 100%">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          {{ scope.row.onlineFaceSignInd ? "是" : "否" }}
        </template>
      </el-table-column>
      <el-table-column
        label="备注">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button @click="uploadImg(scope.row)" type="text" size="small">上传影像</el-button>
        </template>
      </el-table-column>
    </el-table>
    <pagination :page="pageConfig" class="pagination" @jump-page="jump"></pagination>
    <div class="btn-area">
      <el-button class="button  button__width-200 button__blue font__white" @click="getCallInfo" :loading="callLoading">叫号</el-button>
    </div>
    ...
    <el-dialog class="call-number" title="" :visible="callNumberVisible" width="450px" :show-close="false">
      ...
      <div class="call-btn">
        <el-button type="primary" class="width100" @click="callAgain" :loading="recallLoading">重呼</el-button>
...
export default {
  ...
  methods: {
    ...
    // 上传影像
    uploadImg(obj) {
      this.uploadImgEnabel(obj.applicationId)
        .then((data) => {
          this.$showImage(obj.applicationId, '柜面备签');
        })
        .catch(err => {
          err.message && this.confirmFn(err.message, 'error');
        });
    ...
    // 检查是否能上传影像功能
    uploadImgEnabel(appliactionId) {
      return this.$MyFetch.get(spareApi.solving.uploadImgEnabel + appliactionId);
    },
