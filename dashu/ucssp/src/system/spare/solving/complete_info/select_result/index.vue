<template>
  <div class="select-result">
    ...
    <div class="dialog-box__bottom-1200">
      <el-button type="primary" @click="nextStep" class="button__width-200">下一步</el-button>
    </div>
  </div>
</template>
        ...
        getInfo(id) {
          this.$MyFetch.get(spareApi.solving.completeInfo.selectProduction.productInfo + id)
            .then((data = {}) => {
              ...
              this.getTableData(this.applicationId);
            })
            .catch(err => {
              console.log(err);
            });
            ...
            .then((data = {}) => {
              ...
              data.forEach(_item => {
                if (_item.onlineFaceSignInd) {
                  onlineTabData.push(_item);
                }
                offlineTabData.push(_item);
              });
              if (this.sPInfo.onlineFaceInd) {
                this.tableData = onlineTabData;
              }
              if (!this.sPInfo.onlineFaceInd) {
                this.tableData = offlineTabData;
              }
            })
            .catch(err => {
...
<style lang="less" rel="stylesheet/less" scoped>
  ...
  .dialog-box .dialog-box__bottom-1200 {
    height: 60px;
