            ...
            >
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  ...
                  <!-- <span class="el-button el-button--text" @click="exportRepayment(scope.row)">导出还款参考表</span> -->
                </template>
              </el-table-column>
            </el-table>
          </div>
            ...
            <div class="dialog-box__form__group margin-top__20">
              <el-form :model="orgSpecialInfo[0]"
                ...
                <el-form-item label="其他贷款用途说明" prop="loanOtherPurpose" class="el-form-item__25">
                  <el-input
                    maxlength="10"
                    :disabled="!(orgSpecialInfo[0].loanPurposeCode === dCode.loanPurposeCode.other || orgSpecialInfo[0].loanPurposeCode === dCode.loanPurposeCode.other1) || orgSpecialInfo[0].loanPurposeType !== dCode.loanPurposeType.consumption"
                ...
                >
                  <el-select :disabled="amortizationDisabled || isDetail || !openVideo" v-model="orgSpecialInfo[0].amortizationMonth">
                    <el-option
              ...
              <div class="check-bank face2face-detail-style" v-if="isBank1">
                ...
                <template v-else>
                  <repay-account ref="repayAccount1"
            ...
            <div class="dialog-box__form__group margin-top__20">
              <el-form :model="orgSpecialInfo[1]"
                ...
                <el-form-item label="其他贷款用途说明" prop="loanOtherPurpose" class="el-form-item__25">
                  <el-input
                    maxlength="10"
                    :disabled="!(orgSpecialInfo[1].loanPurposeCode === dCode.loanPurposeCode.other || orgSpecialInfo[1].loanPurposeCode === dCode.loanPurposeCode.other1) || orgSpecialInfo[1].loanPurposeType !== dCode.loanPurposeType.consumption"
                ...
                <el-form-item
                  ...
                  class="el-form-item__25"
                >
                  <el-select :disabled="amortizationDisabled || isDetail || !openVideo" v-model="orgSpecialInfo[1].amortizationMonth">
                    <el-option
                      v-for="item in orgSpecialInfoArr[0].amortizationMonthList"
                      :key="item.itemCode"
                      :label="item.itemDesc"
                ...
                <el-form
                  ...
                  <div class="dialog-box__form__group">
                    ...
                    <el-form-item :label="labelMap[1].bankNameLabel" prop="bankName" class="el-form-item__33" >
                      <el-input readonly v-model="cardInfoList[1].bankName" placeholder="请填写收款行名称"></el-input>
                    </el-form-item>
                    <el-form-item :label="labelMap[1].subbankCodeLabel" prop="subbankCode" class="el-form-item__33">
                      <el-input readonly v-model="cardInfoList[1].subbankCode" placeholder="请填写开户行行号"></el-input>
                    </el-form-item>
...
export default {
  components: { specialForm, pagination, DBankPicker, repayAccount, receiveAccount, DQuerySelect },
  props: {
    // 详情传过来的申请编号
    applicationIdDetail: {
      type: String,
  ...
  data() {
    ...
    return {
      ...
      api: {},
      impowerFlag: false, // 企业税 授权控制开关
      queryImpowerBtnStatus: false, // 获取授权按钮 状态
      queryTax: true,
      openTaxInfoDg: false, // 授权结果数据
      taxAgencyList: [], // 税务数据接入机构下拉列表
      tlTaxOrganizationId: '', // 税务数据接入机构
      creditLevelList: [],
      productInfo: {
  ...
  methods: {
    exportRepayment(_row) {
      ...
      let params = {
        ...
        applicationSubmitAmt: _row.applicationSubmitAmt
      };
      this.$MyFetch.post(url, params).then(res => {
        this.$download(res);
      }).catch(_err => {
        ...
        .then(data => {
          ...
          data.bankCardInfoList.forEach((item, index) => {
            ...
            if (item.showElectronicAccountDescInd) {
              this.labelMap[item.sortNum - 1] = map;
            }
    ...
    checkloanOtherPurpose0(rule, value, callback) {
      ...
      if (
        (this.orgSpecialInfo[index].loanPurposeCode === this.dCode.loanPurposeCode.other || this.orgSpecialInfo[index].loanPurposeCode === this.dCode.loanPurposeCode.other1) && this.orgSpecialInfo[index].loanPurposeType === this.dCode.loanPurposeType.consumption && !value
      ) {
        callback(new Error("请填写其他贷款用途说明"));
      } else if (
        (this.orgSpecialInfo[index].loanPurposeCode === this.dCode.loanPurposeCode.other || this.orgSpecialInfo[index].loanPurposeCode === this.dCode.loanPurposeCode.other1) && this.orgSpecialInfo[index].loanPurposeType === this.dCode.loanPurposeType.consumption && !valReg.test(value)
      ) {
      ...
      this.$MyFetch
        ...
        .then(() => {
          ...
          if (_flag) { // 判断是否是提交面核操作true或者保存操作false
            window.eventBus.$emit('faceCheck_submitStopVideo');
            this.isSubmit = false;
          } else {
  ...
  beforeRouteLeave(to, from, next) {
    if (this.isCustomerUpdate) {
      ...
      window.eventBus.$emit("checkTabsFn", "product-info");
      next(false);
    }
  },
  beforeDestroy() {
