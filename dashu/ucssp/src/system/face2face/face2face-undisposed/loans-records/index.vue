<template>
  <div class="loans-records">
    <!-- 详情 -->
    <!-- <face2face-detail :dialogVisible="dialogVisible" v-if="f2fTypeCodeList[openNum] !== '004'" :applicationId="applicationId" @closeDialog="dialogVisible = false"></face2face-detail> -->
    <div class="dialog-box__middle-top">
      <h5 class="title"><span class="icon icon__gan"></span>历史贷款记录</h5>
    </div>
    <loanHistory :applicationId="applicationId" v-show="f2fTypeCodeList[openNum] === '004'"></loanHistory>
    <loanHistoryOffline :applicationId="applicationId" v-show="f2fTypeCodeList[openNum] !== '004'"></loanHistoryOffline>
    ...
    <div v-show="f2fTypeCodeList[openNum] === '004'">
      <div class="dialog-box__middle-top">
        <h5 class="title"><span class="icon icon__gan"></span>关联信息检测</h5>
      </div>
     <detectionMsg :applicationId="applicationId" :isDetail="isDetail"/>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      // 历史贷款记录
      tableData: [],
      openNum: window.openNum, // 获取那一层el-dialog
  ...
  mounted() {
    ...
    // this.openNum = parseInt(sessionStorage.getItem('openNum')) || 0;
  },
  methods: {
    // 获取数据列表
    getList() {
