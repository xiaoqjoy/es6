<template>
  <div class="complete-customer-info face2face-detail-style">
    <div class="dialog-box__middle-form">
      ...
      <div class="form__block bg__white border__radius-5" v-if="!isSurvey">
          <div class="dialog-box__middle-top">
          ...
          <div class="dialog-box__form__group margin-top__20">
            ...
            <ic-info :showSpouse="false"
              :apiUrl="icInfo_url" :applicationId="applicationId" :disabled="(isDetail || !openVideo) && icDisabled"
              :isDetail="isDetail" :routerPath="'counter-ic-info-detail'" :taskNodeCode="'faceToFaceTask'">
            </ic-info>
          </div>
      </div>
      <div class="form__block bg__white border__radius-5" v-if="!isSurvey">
          ...
          <div class="dialog-box__form__group margin-top__20">
            <div class="drop__select-box el-form-item__25">
              ...
              <button
                ...
                :class="{isactive:dropSelectShow3, disabled: isDetail || !openVideo}">{{selectManagerChannel.userName}}<i class="icon el-select__caret el-input__icon el-icon-arrow-up"></i> </button>
            ...
            <el-form-item label="属地客服：" prop="custServiceId" class="el-form-item__25 width220" >
              <el-select filterable v-model="custMarketingInfo.custServiceId" placeholder="请选择属地客服" :disabled="true">
                <el-option v-for="item in localCustemService" :key="item.itemCode" :label="item.itemDesc" :value="item.itemCode"></el-option>
              </el-select>
            </el-form-item>
          </div>
        </el-form>
    ...
    <div class="form__bottom clearfix fixed-save" v-if="!isDetail && openVideo">
      <el-button type="primary" @click="submit(false)" class="width100 button-default">保存</el-button>
    </div>
  </div>
</template>
<script type="text/ecmascript-6">
import "@common/css/dialog";
import { baseApi, autoIc } from "../../js/server.js";
...
export default {
  ...
  data() {
    return {
      ...
      selectArr: ['customer_tax_cd'], // 可配置的下拉获取
      customerTaxCdLibrary: [], // 客户下拉类型
  ...
  created() {
    ...
    let selectArr = [
      'customer_tax_cd', 'gender_code', 'house_condition_code',
    ...
    if (this.isDetail) {
      selectArr.push('pf_marital_status_code');
    } else {
      selectArr.push('marital_status_code');
    }
    this.getMultipleLibrary(selectArr, this.isDetail)
      .then(_data => {
        this.customerTaxCdLibrary = _data.customer_tax_cd;
        this.genderCodeLibrary = _data.gender_code;
        this.maritalStatusCodes = _data.marital_status_code || _data.pf_marital_status_code;
        this.houseConditionCodes = _data.house_condition_code;
        this.careerTypeCodes = _data.career_type_code;
        this.sharePercents = _data.share_percent;
        this.educationLevelLibrary = _data.education_level_cd;
        this.registeredAreaLibrary = _data.registered_area_cd;
        this.organizationTypeLibrary = _data.organization_type_cd;
        this.positionTypeLibrary = _data.position_type_cd;
        this.certificateLibrary = _data.certificate_type_code;
        this.personalTypeCdLibrary = _data.personal_type_cd;
        this.relationshipCodes = _data.family_relationship_code;
        this.norelationshipCodes = _data.non_family_relationship_code;
        this.reportSourceCodes = _data.report_source_code;
        this.authReasonCodes = _data.auth_reason_code;
        this.businessTypeCodes = _data.business_type_code;
      }).catch(_error => {
        this.confirmFn(_error.message);
      });
    this.getPricingPlanApi();
    let rolesList = ['cpms_r_cc_0001'];
    this.getUserList(rolesList, this.subCompanyId);
  },
  methods: {
    delvModuleClass() {
    ...
    submit(_flag) {
      ...
      if (hasPass === 0) {
        ...
        //     this.$parent.$parent.faceCheck_submitStopVideo();
        //     return;
        //   }
        // }
        // console.log("========== 保存接口 =========");
        ...
        this.basicInfo_saveApi(data).then(() => {
          ...
          if (_flag) { // 判断是否是提交面核操作true或者保存操作false
            ...
            // this.$emit("faceCheck_submitStopVideo", '1');
            this.$parent.$parent.faceCheck_submitStopVideo();
            this.isSubmit = false;
          } else {
           ...
           loading.close();
           this.confirmFn(err.message, "success");
        });
      } else {
    ...
    getInfo() {
      this.$MyFetch
        ...
        .then((data) => {
          ...
          if (data.custMarketingInfo.custManagerId) {
            this.getChannelName3(data.custMarketingInfo.custManagerId); // 客户经理转换
          }
          data.custMarketingInfo.custServiceId = data.custMarketingInfo.custServiceId || '00000';
  ...
  //   };
  //   if (!this.isCustomerUpdate) {
  //     this.$emit("checkTabsFn", "customers-info", data);
  //   }
  // },
  ...
  beforeRouteLeave(to, from, next) {
    ...
    if (this.isCustomerUpdate) {
      ...
      window.eventBus.$emit("checkTabsFn", "customers-info");
      next(false);
    }
  },
  beforeDestroy() {
