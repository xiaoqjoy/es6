    ...
    <div class="table-area">
      ...
      <div class="tableHeight">
        <el-table :data="tableData" style="100%" class="common__table">
          ...
          <el-table-column prop="authReasonDesc" label="授权原因"></el-table-column>
          <el-table-column prop="f2fTypeDesc" label="面签类型"></el-table-column>
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <!-- <el-button class="button-text" @click="showCheck(scope.row.applicationId,scope.row.customerName,scope.row.resignInd)">面签面核</el-button> -->
              <el-button class="button-text" type="text" @click="todo(scope.row.applicationId,scope.row.customerName,scope.row.resignInd,scope.row.easyLoanInd,scope.row.approveModeCd, scope.row)">面签面核</el-button>
              <el-button class="button-text" type="text" @click="handUp(scope.row.applicationId)">挂起</el-button>
            </template>
          </el-table-column>
        </el-table>
      ...
      <div class="bottom-btn" v-if="!isXiuwen">
        ...
        <!-- <el-button @click="visaVisible = !visaVisible">测试面核面签</el-button> -->
      </div>
...
export default {
  ...
  created() {
    ...
    // this.getCallInfo();
    console.log('初始化话术库，是否开启录像');
  },
  beforeDestroy() {
    console.log('销毁getMac');
  ...
  methods: {
    todo(_applicationId, _customerName, _resignInd, easyLoanInd, approveModeCd, _row) {
      let ydApproveMode2 = easyLoanInd && approveModeCd === '2';
      let {subCompanyId, f2fTypeCode} = _row;
      this.pendingStart(_applicationId).then((res) => {
        if (res) {
          this.confirmFn(res, 'success').then(() => {
            this.$router.push({
              ...
              params: {
                ...
                resignInd: _resignInd,
                ydApproveMode2: ydApproveMode2
              },
              query: {
                subCompanyId: window.btoa(subCompanyId),
                f2fTypeCode: window.btoa(f2fTypeCode)
              }
            });
          });
        } else {
      ...
      //   }
      // });
      // const markTab = markOpenTab.markOpen();
      // if (!markTab) {
      //   this.confirmFn('您已经打开一个业务单了，请关闭之后再打开', 'error');
      //   return;
      // }
    ...
    // 显示并开始面核面签
    showCheck(applicationId, customerName, resignInd) {
