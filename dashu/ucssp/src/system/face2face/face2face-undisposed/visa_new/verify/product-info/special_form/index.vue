    ...
    <div class="special_form-page">
      <el-form
        ...
        <!-- <div  class="special_item clearfix"> -->
          <el-form-item v-for="item in renderArr" :key="item.orgItemCode"
            ...
            <el-select
              ...
              <el-option
                v-for="item in library[item.orgItemCode]"
                :key="item.itemCode"
                :label="item.itemDesc"
                :value="item.itemCode"
              ></el-option>
      ...
      dellArr() {
        this.renderArr.forEach((v) => {
          ...
          if (v.ruleJson.funs.length > 0) {
            ...
            v.ruleJson.funs.forEach((k, i) => {
              let obj = {};
              // 需要改变this的指向
              if (v.orgItemCode === 'jnyh_operator_name') {
                k.fun = "if(this.$parent.$parent['orgSpecialInfo'][this.formName.replace('formMore','')-1].loanPurposeType==='020'){if(!value) {return callback(new Error('法人姓名不能为空'))}else if(value.length>60){return callback(new Error('法人姓名超过60字符'))}else {return callback();}}else{return callback()}";
              } else if (v.orgItemCode === "jnyh_three_certificate_code") {
                k.fun = "if(this.$parent.$parent['orgSpecialInfo'][this.formName.replace('formMore','')-1].loanPurposeType==='020'){if(!value) {return callback(new Error('统一社会信息代码不能为空'))}else if(value.length>50){return callback(new Error('统一社会信息代码超过50字符'))}else {return callback();}}else{return callback()}";
              }
              // eslint-disable-next-line
      ...
      relativeFn(arr = [], selfCode) {
        ...
        arr.forEach(v => {
          v.disabledArr.forEach(k => {
            if (v.parent) {
              let index = this.formName.replace("formMore", "") - 1;
              if (this.$parent.$parent["orgSpecialInfo"][index][v.relativeCode] === k) {
                isDisabled = true;
