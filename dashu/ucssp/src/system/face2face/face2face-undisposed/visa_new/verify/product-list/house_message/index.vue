    ...
    <div class="dialog-box">
      ...
      <div class="dialog-box__middle">
          ...
          <div class="form__block bg__white border__radius-5" >
            ...
            <div class="dialog-box__form__group">
              ...
              <el-form-item label="房产证地址" prop="addressCode">
                ...
                <select-cascader v-model="houseEdit.addressCode" :options="areaList" :disabled="!!hasSubmit" @change="inputChange"></select-cascader>
              </el-form-item>
              ...
              <el-form-item label="房屋性质" prop="estateTypeCode">
                <el-select
                  ...
                  <el-option
                    v-for="item in estateTypeCodeLibrary"
                    :key="item.itemCode"
                    :label="item.itemDesc"
            ...
            <div class="dialog__table-form-box">
              <ul class="table__box">
                ...
                <li class="table__body" v-for="(item, index) in coownerList" :key="index">
                  <div class="table__tr">
                    <ul class="table__tr__top-list" v-show="!item.showEle">
                      ...
                      <li class="table__td">{{item.certificateTypeCode | certificateFilter(certificateTypeCodeLibrary)}}</li>
                      ...
                      <div class="table__tr__bottom-list" v-show="item.showEle">
                        ...
                        <el-form-item prop="certificateTypeCode" class="table__form-item">
                          <el-select v-model="coownerEdit.certificateTypeCode" placeholder="请选择">
                            <el-option v-for="(item, index) in certificateTypeCodeLibrary"
                              :key="index"
                              :value="item.itemCode"
                              :label="item.itemDesc"></el-option>
                          </el-select>
                        </el-form-item>
                        <el-form-item style="flex: 1 1 90px;"
                          prop="certificateNum"
                ...
                <li class="table__body" v-show="coownerAddDg || (coownerList.length === 0)">
                  <div class="table__tr">
                    <el-form :model="coownerEdit"
                        v-if="!isDetail" :rules="coownerRule"
                        ref="coownerAddForm"
                        label-width="100px"
                        class="demo-form-inline" label-position="top" >
        ...
        <div class="form__bottom" v-if="!isDetail">
          <button type="button" @click="saveHouse(true)" class="width180 button__border-blue button" :disabled="!!hasSubmit">保存</button>
          <button type="button" @click="querySl" class="width180 button__yellow button" :disabled="!!hasSubmit">世联查询</button>
          <button type="button" @click="queryResultSl" class="width180 button__green button" :disabled="!!hasSubmit">世联结果查询</button>
          <el-button type="primary" class="width180" @click="saveHouse(false)" :disabled="!!hasSubmit">确认</el-button>
        </div>
      </div>
    </div>
  </div>
...
import { coownerRule, houseRule } from '../../../../../js/mind';
...
export default {
  ...
  data() {
    ...
    return {
      ...
      coownerList: [],
      coownerRule: Object.assign(coownerMixRule, coownerRule),
      coownerAddDg: false,
      // 房屋信息字段
      houseRule: Object.assign(houseRule, houseMixRule),
      houseEdit: {
        estateNum: '',
        addressCode: [],
        addressDesc: '',
  ...
  created() {
    // console.log(this.estateTypeCodeLibrary);
  ...
  filters: {
    certificateFilter(_val, _list) {
      const find = _list.find(_i => {
        return _i.itemCode === _val;
      });
  ...
  methods: {
    ...
    close() {
     ...
     // 共享人信息
    openAddCoowner() {
      let isEditing = this.isEditing(this.coownerList);
    ...
    saveCoowner() {
      this.$refs.coownerAddForm.validate((valid) => {
        if (valid) {
          let data = Object.assign(this.coownerEdit);
    ...
    saveCoownerEdit(formName, index) {
      this.$refs[formName][0].validate((valid) => {
        if (valid) {
          ...
          // console.log('error');
          this.confirmFn('表单字段未填写完整', "error");
        }
      });
    },
    // 删除共享人信息项
    deleteCoowner(index) {
      this.confirmFn('是否删除该条信息！', 'warning').then(() => {
        ...
        this.$MyFetch.get(baseApi.completeProduct.deletCoowner + this.coownerList[index].id)
          .then(data => {
    ...
    saveHouseApi(data, temporary = false) {
      this.$MyFetch.post(baseApi.completeProduct.addHouse, data)
        .then((data) => {
    ...
    saveEditHouseApi(data, temporary = false) {
      this.$MyFetch.post(baseApi.completeProduct.editHouse, data)
        .then(() => {
    ...
    // 获取房屋信息
    getHouseMessage(id) {
      this.$MyFetch.get(baseApi.completeProduct.getHourseMessage + id)
        .then((data = {}) => {
    ...
    querySl() {
      ...
      this.$refs.houseForm.validate((valid) => {
        if (valid) {
          this.$MyFetch.post(baseApi.completeProduct.getSl, data)
            .then((data) => {
              if (data.isSuccess) {
    ...
    queryResultSl() {
      ...
      this.$MyFetch.post(baseApi.completeProduct.getManual, data)
        .then((data) => {
          if (data.isSuccess) {
            this.confirmFn('查询成功！', 'success');
            this.houseEdit.slAverageAmt = data.unitPrice;
    ...
    formatMoney() {
      this.houseEdit.houseSizeMeas = this.$returnFloat(this.houseEdit.houseSizeMeas, 2);
    },
    changeCoowner() {
