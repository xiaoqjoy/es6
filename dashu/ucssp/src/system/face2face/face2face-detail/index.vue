<template>
  <el-dialog
   class="face2face-detail" title="详情"
  :visible="dialogVisible" :width="width + 'px'"
  @open="openFn" @close="closeDialog" :modal-append-to-body="false"
      ...
      <div class="nav-content1" v-if="currentTab === 0">
        <div class="check-tabs">
          <el-tabs v-model="checkTabs" @tab-click="checkTabsClick">
            ...
            <el-tab-pane label="产品收入信息" name="product-list" v-if="!isSurvey"></el-tab-pane>
            <el-tab-pane label="产品信息" name="product-info" v-if="!isSurvey"></el-tab-pane>
          ...
          <div class="check-tabs-btn">
            ...
            <div class="button-text" @click="$showImage(applicationId, '柜面面签', {ReadOnly: '0'})">影像资料</div>
            <!-- <el-button type="text" @click="showCredit">查看征信报告</el-button> -->
            <view-credit-report :moduleCode="'counterF2F'" :applicationId="applicationId" :isEdit="true" @showCredit="showCredit1"></view-credit-report>
          </div>
        </div>
        <div class="check-component">
          <component :subCompanyId='subCompanyId' :is="checkTabs" :applicationIdDetail="applicationId" :isDetail="true" :isSurvey="isSurvey"></component>
      ...
      <div class="nav-content2" v-if="currentTab === 1 && isSubmit">
        ...
        <div class="check-remark">
          <div class="conclusion" style="padding: 20px 0">
            <label>外网查询结论：</label>
            <el-select placeholder="请选择" v-model="check_resultData.aplExtranetInfo.extranetStatusCode" :disabled="true">
              <el-option v-for="item in extranetStatusCodes" :key="item.itemCode" :label="item.itemDesc" :value="item.itemCode"></el-option>
            </el-select>
          </div>
          <div class="remark">
            <el-input type="textarea" maxlength="500" placeholder="请输入外网查询备注" :rows="5" v-model="check_resultData.aplExtranetInfo.remark" :disabled="true"></el-input>
          </div>
        </div>
      </div>
    </div>
...
import productList from "../face2face-undisposed/product_list/index"; // 历史贷款记录组件
