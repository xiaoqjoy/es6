          ...
          <el-col>
            ...
            <div style="padding: 20px;">
              ...
              <!-- <div class="bottom clearfix">
                <span class="webSite">{{item.link}}</span>
              </div> -->
            </div>
          </el-col>
        </el-row>
...
export default {
  ...
  methods: {
    ...
    // 编辑页面跳转
    compile(data) {
      let url = `${partnerApi.banner}/${data.id}`;
      this.$MyFetch
        .get(url)
        .then((data = {}) => {
    ...
    refer(pageNum) {
      let data = {
        current: pageNum, // 当前页
        size: this.pageConfig.pageSize // 当前多少条数据
      };
      let url = partnerApi.banner;
      this.$MyFetch
        .get(url, data)
        .then((data = {}) => {
