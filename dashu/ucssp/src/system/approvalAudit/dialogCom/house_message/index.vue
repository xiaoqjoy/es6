  ...
  export default{
    ...
    methods: {
      ...
      saveHouse(temporary = false) {
        this.$refs.houseForm.validate((valid) => {
          if (valid) {
            ...
            // data.incomeInd = data.incomeInd ? data.incomeInd : true;
      ...
      saveHouseApi(data, temporary = false) {
       this.$MyFetch.post(aprAudApi.completeProduct.addHouse, data)
         .then((data) => {
      ...
      saveEditHouseApi(data, temporary = false) {
        this.$MyFetch.post(aprAudApi.completeProduct.editHouse, data)
          .then(() => {
      ...
      // 获取房屋信息
      getHouseMessage(id) {
        this.$MyFetch.get(aprAudApi.completeProduct.getHourseMessage + id)
          .then((data = {}) => {
      ...
      querySl() {
        ...
        this.$refs.houseForm.validate((valid) => {
          if (valid) {
            this.$MyFetch.post(aprAudApi.completeProduct.getSl, data)
              .then((data) => {
                if (data.isSuccess) {
      ...
      queryResultSl() {
        ...
        this.$MyFetch.post(aprAudApi.completeProduct.getManual, data)
          .then((data) => {
            if (data.isSuccess) {
              this.confirmFn('查询成功！', 'success');
              this.houseEdit.slAverageAmt = data.unitPrice;
