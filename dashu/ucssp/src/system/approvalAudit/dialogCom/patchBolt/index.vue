  ...
  methods: {
    ...
    backPatch(data) {
      return new Promise((resolve, reject) => {
        ...
        let data = {
          patchClassItems: this.configCode,
    ...
    returnPatch() {
      ...
      for (let i = 0; i < this.sendPatchBoltValue.length; i++) {
        if (this.sendPatchBoltValue[i].configCode === "C01-01") {
          flag = true;
        }
        this.configCode.push({
          siCategoryCd: this.sendPatchBoltValue[i].configCode,
          siDetailTxt: this.sendPatchBoltValue[i].configTitle
        });
        this.patchContent.push(this.sendPatchBoltValue[i].configDesc);
        this.patchContentTxt = this.patchContent.join("|");
      }
      if (flag && this.configCode.length > 1) {
        this.$message({
          message: '请单独选择“额度30万以上业务请银行电核”进行补件下发！',
          type: 'warning'
        });
        return;
      }
