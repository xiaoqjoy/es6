...
import comebackA from "../../../dialogCom/comebackA"; // 退回一
import comebackB from "../../../dialogCom/comebackB"; // 退回一
import comebackC from "../../../dialogCom/comebackC"; // 退回一
...
export default {
  ...
  data() {
    ...
    return {
      ...
      ruleForm: {
        ...
        houseFund6mInd: false, // boolean6个月内是否正常缴纳公积金
  ...
  created() {
    // this.getIsEasyLoan();
    this.getSelectInfo();
    this.getMultiDict(['task_def_key', 'approve_mode', 'insurance_company']);
  ...
  methods: {
    getRemaind(channel) {
      this.$MyFetch.get(aprAudApi.aud.remaind + "/" + this.applicationId).then((data) => {
          ...
          for (let k = 0; k < value.length; k++) {
            if (value[k] === 'S01-01') {
              for (let i = 0; i < this.riskResult.length; i++) {
                if (this.riskResult[i].level2Code === 'S01-02') {
                  this.riskResult[i].disabled = true;
                }
              }
            }
          }
    ...
    //   // this.dontChange(value);
    //   this.selectSpan = this.getTypesName(JSON.parse(JSON.stringify(value)));
    //   // let arr = this.riskResult.find(function(elem) {
    //   //   return elem.level2Code === 'S01-01';
    //   // });
    //   // arr.disabled = true;
    //   // console.log(this.selectSpan, value);
    // },
    getTypesName(value) {
      let arr = [];
      for (let i = 0; i < this.riskResult.length; i++) {
    ...
    gettaskDetail() {
      return new Promise((resolve, reject) => {
        let url = '';
        if (this.isEdit) {
          url = `${aprAudApi.aud.taskDetail.taskDetailReadonly}/${this.applicationId}`;
        } else {
          url = `${aprAudApi.aud.taskDetail.taskDetail}/${this.applicationId}`;
        }
        this.$MyFetch
          .get(url)
          .then((data = {}) => {
            ...
            this.getPhoneCheckConfig(data.personalInfo.careerTypeCode, code); // 获取电核结论里面的配置
    ...
    saveOperAll() {
      ...
      if (!this.audConclusion.audExtranetStatusCode) {
        isPass += 1;
      ...
      if (isPass === 0) {
        ...
        let data = {
          ...
          audExtranetStatusCode: this.audConclusion.audExtranetStatusCode,
          audExtranetRemark: this.audConclusion.audExtranetRemark
        };
    ...
    //   //   .then((data = {}) => {
    //   //     // console.log(data, '电核结论配置');
    //   //     this.riskResult = data;
    //   //     for (let i = 0; i < this.riskResult.length; i++) {
    //   //       this.riskResult[i].disabled = false;
    //   //     }
    ...
    getRuleDetection() {
      ...
      // }
      this.$MyFetch
        .get(`${aprAudApi.aud.taskDetail.ruleDetection}/${this.applicationId}`, data)
        .then((data = {}) => {
    ...
    getRiskPara() {
      let url = '';
      if (this.isEdit) {
        url = `${aprAudApi.aud.taskDetail.riskParaDataReadonly}/${this.applicationId}`;
      } else {
        url = `${aprAudApi.aud.taskDetail.getRiskParaData}/${this.applicationId}`;
      }
      this.$MyFetch
        .get(url)
        .then((data = {}) => {
    ...
    _riskPara(url) {
      ...
      this.$confirm(message, {
        ...
        this.loadingOpen();
        this.$MyFetch
          .post(`${aprAudApi.aud.taskDetail.riskParaSaveOper}/${url}/${this.applicationId}`, data)
          .then((data = {}) => {
            this.loadingClose();
            if (this.isDadao) {
    ...
    // 获取第三方数据
    getTreeData(v) {
      let url = `${aprAudApi.aud.taskDetail.threedataList}/${this.applicationId}`;
      this.$MyFetch
        .get(url)
        .then((data = {}) => {
          // this.ruleDetection = data;
          let orangeList = JSON.parse(data);
          let personList = [];
          let companyList = [];
          for (var i = 0; i < orangeList.data.length; i++) {
            if (orangeList.data[i].search_type === 1) {
              if (orangeList.data[i].hasOwnProperty("fxmsgnum")) {
                orangeList.data[i].totalnumber = orangeList.data[i].fxmsgnum;
              }
              personList.push(orangeList.data[i]);
            }
            if (orangeList.data[i].search_type === 2) {
              if (orangeList.data[i].hasOwnProperty("n")) {
                orangeList.data[i].q = orangeList.data[i].n;
                orangeList.data[i].totalnumber = orangeList.data[i].fxmsgnum;
              }
              companyList.push(orangeList.data[i]);
            }
          }
