...
<div class="drop_loan">
  ...
  <div class="dialog-box">
      ...
      <div class="icon_btn_line" v-if="!isSurveying && isPreview && notApprovalAudit">
		...
		<self-employed v-if="reportType===1 && !isPreview && isTwice" @changeReportType="changeReportType" @viewToPreview="viewToPreview" ref="reportContent" :reportData="reportData" :custInfo="custInfo"></self-employed>
    <salaried-personnel v-if="reportType===2 && !isPreview && isTwice" @changeReportType="changeReportType" @viewToPreview="viewToPreview" ref="reportContent" :reportData="reportData" :custInfo="custInfo"></salaried-personnel>
    <report-preview v-if="isPreview" :reportData="reportData" :custInfo="custInfo"></report-preview>
...
export default {
  ...
  methods: {
    // 第二次获取报告数据
    getTwiceReportData(loadingInstance) {
      this.$MyFetch.get(surveyApi.getinterface + this.surveyId).then(date => {
        this.$nextTick(() => { // 以服务的方式调用的 Loading 需要异步关闭
          loadingInstance.close();
        });
        this.isTwice = true;
        this.reportData = JSON.parse(JSON.stringify(date));
        if (this.reportData.carInfoList === null) {
          this.reportData.carInfoList = [];
        }
        if (this.reportData.estateInfoList === null) {
          this.reportData.estateInfoList = [];
        }
        ...
        // 若第二次调用接口无数据 则采用第一次调用数据
        if (this.reportData.svyBossReport !== null && this.reportData.svyStaffReport === null) {
          this.reportData.svyStaffReport = JSON.parse(JSON.stringify(this.firstReportData.svyStaffReport));
        } else if (this.reportData.svyStaffReport !== null && this.reportData.svyBossReport === null) {
          this.reportData.svyBossReport = JSON.parse(JSON.stringify(this.firstReportData.svyBossReport));
        } else if (this.reportData.svyBossReport === null && this.reportData.svyStaffReport === null) {
          this.reportData = JSON.parse(JSON.stringify(this.firstReportData));
        }
        if (this.isLookOver) {
        this.isPreview = true;
      }
    ...
    getReportData() {
      ...
      this.$MyFetch.get(url).then(data => {
        ...
        this.firstReportData = JSON.parse(JSON.stringify(data));
        ...
        // if ((!data.svyBossReport && !data.svyStaffReport) || (data.svyBossReport.surveyId === null && data.svyStaffReport.surveyId === null)) {
        if (!data.svyBossReport && !data.svyStaffReport) {
          this.getTwiceReportData(loadingInstance);
        } else if (data.svyBossReport && !data.svyStaffReport) {
          if (data.svyBossReport.surveyId === null) {
            this.getTwiceReportData(loadingInstance);
          }
        } else if (data.svyStaffReport && !data.svyBossReport) {
          if (data.svyStaffReport.surveyId === null) {
            this.getTwiceReportData(loadingInstance);
          }
        } else if (data.svyStaffReport && data.svyBossReport) {
          if (data.svyStaffReport.surveyId === null && data.svyBossReport.surveyId === null) {
            this.getTwiceReportData(loadingInstance);
          }
        }
        if (data.svyBossReport === null && data.svyStaffReport !== null) {
            if (data.svyStaffReport.surveyId) {
            this.isTwice = true;
          }
        } else if (data.svyStaffReport === null && data.svyBossReport !== null) {
            if (data.svyBossReport.surveyId) {
            this.isTwice = true;
          }
        } else {
          if (data.svyBossReport.surveyId || data.svyStaffReport.surveyId) {
            this.isTwice = true;
          }
        }
        this.initReport();
