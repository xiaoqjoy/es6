<template>
  <div class="survey_task_list">
    ...
    <el-form class="search" :inline="true" :model="searchInfo" ref="searchInfo" :rules="searchRule">
      ...
      <el-form-item prop="taskTriggerTime">
        <el-date-picker v-model="searchInfo.taskTriggerTime" value-format="yyyy-MM-dd HH:mm:ss" type="datetimerange" start-placeholder="任务触发时间(起)" end-placeholder="任务触发时间(终)" @change="changeTriggerTime"></el-date-picker>
      </el-form-item>
      <el-form-item prop="finishSurveyTime">
        <el-date-picker v-model="searchInfo.finishSurveyTime" value-format="yyyy-MM-dd HH:mm:ss" type="datetimerange" start-placeholder="调查完成时间(起)" end-placeholder="调查完成时间(终)" @change="changeFinishTime"></el-date-picker>
      </el-form-item>
      <el-form-item>
    ...
    <div class="table_box">
      ...
      <el-table :data="pageData.list" style="100%">
        ...
        <el-table-column prop="finishSurveyTime" label="调查完成时间" width="160"></el-table-column>
        ...
        <el-table-column prop="surveyDistance" label="距离" width="100"></el-table-column>
        ...
        <el-table-column prop="surveyConclusion" label="调查结论" width="110"></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      searchInfo: {
        ...
        pageSize: 10,
        subcompanyName: '',
        surveyStatus: 'limit_invalid',
        surveyManagerName: '',
        taskTriggerStartTime: '', // 任务触发时间（起）
        taskTriggerEndTime: '',
        taskTriggerTime: '',
        finishSurveyStartTime: '',
        finishSurveyEndTime: '', // 调查完成时间（止）
        finishSurveyTime: '', // 完成时间段数组
        account: 0,
        currentPage: 1,
        showJumpButton: true
      },
  ...
  methods: {
    ...
    changeFinishTime(val) {
      if (val) {
        this.searchInfo.finishSurveyStartTime = val[0];
        this.searchInfo.finishSurveyEndTime = val[1];
      } else {
        this.searchInfo.finishSurveyStartTime = '';
        this.searchInfo.finishSurveyEndTime = '';
      }
    },
    // 查询
    search() {
    ...
    clearSearch() {
      ...
      this.taskTriggerTime = [];
      this.finishSurveyTime = [];
      this.searchInfo.taskTriggerStartTime = '';
      this.searchInfo.taskTriggerEndTime = '';
      this.searchInfo.finishSurveyStartTime = '';
      this.searchInfo.finishSurveyEndTime = '';
    ...
    jumpToDetail(row) {
      ...
      this.$store.commit('set_surveySource', row.businessSourceCode);
      this.$router.push({
        name: 'limit-failure-detail',
        params: {
          surveyId: row.surveyId,
          applicationId: row.applicationId,
          surveySource: row.businessSourceCode
        }
      });
    }
  },
