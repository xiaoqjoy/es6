...
<template>
  <div>
    <div class="container" v-if="!monitoringSwitch">
            ...
            <template v-else>
              <div class="date-group">
                <el-date-picker
                  ...
                  type="date"
                  value-format="yyyy-MM-dd"
                  v-model="form[item.code][0]"
                ></el-date-picker>-
                <el-date-picker
                  placeholder="终止时间"
                  type="date"
                  value-format="yyyy-MM-dd"
                  v-model="form[item.code][1]"
                ></el-date-picker>
              </div>
            </template>
          </template>
     ...
     <datamonitoring v-if="monitoringSwitch" />
  </div>
</template>
<script>
import { reportsApi } from "./js/server";
import pagination from "@components/pagination";
import datamonitoring from "./data-monitoring/index";
...
export default {
  watch: {
    $route: {
      handler: function() {
        ...
        if (this.$route.name === "data-monitoring") {
          this.monitoringSwitch = true;
        } else {
          this.monitoringSwitch = false;
          this.reset();
          this.getConfig();
        }
      },
  ...
  methods: {
    ...
    getConfig() {
      this.$MyFetch
        .get(reportsApi.query.replace("{reportCode}", this.reportCode))
        .then(data => {
    ...
    getOptions(code) {
      this.$MyFetch
        .get(reportsApi.select.replace("{categoryCode}", code))
        .then(data => {
    ...
    getData(pageNum = 1) {
      ...
      this.$MyFetch
        .post(
          `${reportsApi.reports}?pageNum=${pageNum}&pageSize=${
            this.pageConfig.pageSize
          }`.replace("{reportCode}", this.reportCode),
          formArr
        )
        .then((data = {}) => {
          if (data) {
    ...
    exportData() {
      ...
      this.$MyFetch
        .post(
          reportsApi.export.replace("{reportCode}", this.reportCode),
          this.makeFormArr(),
          "blob"
        )
        .then(res => {
