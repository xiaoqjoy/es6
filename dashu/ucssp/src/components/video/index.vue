...
export default {
  ...
  data() {
    return {
      isOpenVideo: true, // 是否开启千里眼（查询处理中心千里眼开关接口）
  ...
  methods: {
    ...
    stopVideo() {
      ...
      // this.isVideo为true时，说明视频已经产生，再点击则不用停止录像，直接记录一笔视频录像业务单，否则停止录像报错（改为不录制视频，直接跳过）
      if (this.isVideo) {
