<template>
  <div class="login-page">
    <div class="box">
      <div class="logo">
        <img src="../../../static/loginlogo.png" height="60" width="120">
      </div>
      <div class="form">
        ...
        <div class="line input" v-if="loginData.id">
          <input type="text" v-model="loginData.gVCode" maxlength="6" placeholder="验证码" @keyup.enter="login()"/>
        ...
        <div class="line link">
          <a href="http://uupm.dashuf.com/umManage/changePwd.html" target="_blank" style="font-size: 12px; color:#666; float: right;">修改密码</a>
          <div>&nbsp;</div>
        </div>
        <div class="line link">
          <a href="http://uupm.dashuf.com/umManage/forgetPwd.html" target="_blank" style="font-size: 12px; color:#666; float: right;">忘记密码</a>
          <div>&nbsp;</div>
        </div>
      </div>
    </div>
  ...
  export default{
    ...
    created() {
      ...
      if (this.$route.params.userId) {
        this.loginData.userId = this.$route.params.userId.toLowerCase();
      }
    },
    methods: {
      login() {
        ...
        if (!this.loginData.gVCode && this.loginData.id) {
          this.hint = '请输入验证码';
          return;
        }
        let sendData = JSON.parse(JSON.stringify(this.loginData));
        sendData.id = sendData.id || false;
        ...
        this.$MyFetch.post(api.login, sendData)
          ...
          .catch(err => {
            if (err.code === 'UM100002' || err.code === 'UM100003') {
              this.getCode();
              this.loginData.gVCode = "";
            }
            this.confirmFn(err.message);
          });
      },
      getCode() {
        this.$MyFetch.post(`${api.verifyCode}?systemId=cpms.ucssp`)
          .then((data) => {
            this.codeUrl = data.imageurl;
            this.$set(this.loginData, 'id', data.id);
          })
          .catch((err) => {
