...
<script>
// 本工具使用的是 ele-multi-cascader 修改过来完成
// 原作者：webCoderJ
// github： https://github.com/webCoderJ/ele-multi-cascader
// 修改之后支持在控件展示想要的字段
// 使用方法： showId="path" 将展示path的字段
//   <ele-multi-cascader
//   class=""
...
function deepClone(source) {
  if (!source && (typeof source !== "object" || source === null)) {
    throw new Error("error arguments, expected Object or Array, but got undefined or null", "shallowClone");
  }
  ...
  methods: {
    ...
    recursiveOpt(children) {
      ...
      children.forEach(child => {
        ...
        if (this.selectedValues.some(val => val + "" === child.value + "")) {
          ...
          this.selectedLabels.push(child[vm.showId]);
          // modify end
          child.checked = true;
    ...
    selectChange(val) {
      ...
      let deletedItem = this.selectedItems.filter(
        ...
        item => !val.includes(item[this.showId])
      )[0];
      ...
      this.selectedItems = this.selectedItems.filter(v =>
        ...
        this.selectedLabels.includes(v[this.showId])
      );
    ...
    checkedChange(item, index, checked) {
      ...
      this.selectedItems.forEach(item => {
        ...
        this.selectedLabels.push(item[this.showId]);
        // modify end
