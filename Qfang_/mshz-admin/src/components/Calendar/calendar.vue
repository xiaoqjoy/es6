    ...
    <table cellpadding="5">
      ...
      <tbody>
        <tr v-for="(day, k1) in days" :key="k1" style="{'animation-delay',(k1*30)+'ms'}">
          <td v-for="(child,k2) in day" :key="k2" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,child.disabled,$event)">
            <span :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}">{{child.day}}</span>
            <div class="text" :class="{'event-red':child.spePrice}" v-if="child.price">
              ￥{{child.price}}
            </div>
            <div class="text-lunarInfo" :class="{'isLunarFestival':child.isLunarFestival,'isGregorianFestival':child.isGregorianFestival}" v-if="lunar">
              {{/节|旦|夜/.test(child.lunar)?child.lunar:''}}
            </div>
          </td>
        </tr>
      </tbody>
  ...
  methods: {
    init() {
      ...
      if (this.value.length > 0) {
        if (this.range) {
          // 如果 起始位置固定，那么找到后面的第一个disabled，把它变成abled，把他之后的变为disabled；
          if (this.fixedStart) {
            this.fixedStartInit();
          }
          // 范围
    ...
    render(y, m) {
      ...
      let prevYear = this.computedPrevYear();
      let prevMonth = this.computedPrevMonth(true);
      let nextYear = this.computedNextYear();
      let nextMonth = this.computedNextMonth(true);
      ...
      for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (this.range) {
          ...
          // 匹配上次选中的日期
          if (
            parseInt(seletSplit[0], 10) === this.year &&
            parseInt(seletSplit[1], 10) - 1 === this.month &&
            parseInt(seletSplit[2], 10) === i
          ) {
    ...
    notFixedStartFirstClick(k1, k2) {
      // 第一次点击后 该日期之后的第一个禁用日期 变为可点，第一个禁用日期之后的不可点
      let tempArr = Object.keys(this.events).sort();

      tempArr.forEach((item, index) => {
        if (this.getYmd(item) === [this.year, this.month + 1, this.days[k1][k2].day].join('-')) {
          this.midKey = index;
        };
      });

      const tempArrRight = tempArr.slice(this.midKey, tempArr.length);
      const tempArrLeft = tempArr.slice(0, this.midKey);

      for (let i = 0; i < tempArrRight.length; i++) {
        if (this.events[tempArrRight[i]].disabled === true) {
          this.rightKey = i;
          break;
        }
      }

      for (let i = tempArrLeft.length - 1; i >= 0; i--) {
        if (this.events[tempArrLeft[i]].disabled === true) {
          this.leftKey = i;
          break;
        }
      }

      this.rightKey = this.rightKey + this.midKey;

      for (let i = 0; i < tempArr.length; i++) {
        if (i < this.leftKey) {
          this.events[tempArr[i]].disabled = true;
        }
        if (i === this.rightKey) {
          this.events[tempArr[i]].disabled = false;
        }
        if (i > this.rightKey) {
          this.events[tempArr[i]].disabled = true;
        }
    ...
    fixedStartInit() {
      // 找到默认结束日期 之后 的第一个禁用日期 变为可点，第一个禁用日期之后的不可点
      let tempArr = Object.keys(this.events).sort();

      tempArr.forEach((item, index) => {
        if (item === this.value[1].join('-')) {
          this.midKey = index;
        };
      });
      ...
      for (let i = 0; i < tempArrRight.length; i++) {
        if (this.events[tempArrRight[i]].disabled === true) {
          this.rightKey = i;
          // console.log(this.events, this.rightKey);

          break;
        }
      }
      if (!this.rightKey) { // 如果没找到 默认结束日期之后的第一个禁用日期
        this.rightKey = tempArrRight.length - 1;
      }
      this.rightKey = this.rightKey + this.midKey;

      this.events[tempArr[this.rightKey]].disabled = false;

      for (let i = this.rightKey; i < tempArr.length; i++) {
        if (i > this.rightKey) {
          this.events[tempArr[i]].disabled = true;
        }
      }
    ...
    // 选中日期
    select(k1, k2, disabled, e) {
      if (e !== undefined) e.stopPropagation();
      if (disabled) {
        return;
      }
      let year = this.days[k1][k2].year;
      let month = this.days[k1][k2].month - 1;
      let day = this.days[k1][k2].day;
      // console.log(year, month, day, 'position:', this.month);
      if (month > this.days[2][3].month - 1) {
        this.next(e);
      }
      if (month < this.days[2][3].month - 1) {
        this.prev(e);
      ...
      if (this.range) {
        // 固定起始日期
        if (this.fixedStart) {
          this.rangeBegin = [parseInt(this.value[0][0], 10), parseInt(this.value[0][1], 10) - 1, parseInt(this.value[0][2], 10)];
          this.rangeEndTemp = 0;
        }
        if (this.rangeBegin.length === 0 || this.rangeEndTemp !== 0) {
          ...
          this.rangeEndTemp = 0;
          if (!this.fixedStart) {
            this.notFixedStartFirstClick(k1, k2);
          }
        } else {
          this.rangeEnd = [year, month, day];
          this.rangeEndTemp = 1;
    ...
    // 筛选节假日
    handleFestivalList(year) {
      // 检查是不是闰年 返回或是当前的闰月 如是 0 则不是闰年
      var leapMonth = calendar.leapMonth(year);
      var festivalList = [];
      for (const key in this.festival.lunar) {
        // 把农历节假日 '1-1' 切割为 月 日 传入
        var keylist = key.split('-');
        // 农历装换公历后的 返回[y, m, d]
        var newDate = calendar.lunarTogregorian(+year, +keylist[0], +keylist[1], (+keylist[0] === +leapMonth));
        var newDateString = newDate.join('-');
        festivalList.push({
          festivalName: this.festival.lunar[key],
          date: newDate,
          dateString: newDateString,
          dateTime: (new Date(+newDate[0], +newDate[1], +newDate[2])).getTime()
        });
      }
      for (const key in this.festival.gregorian) {
        var key2list = key.split('-');
        key2list.unshift(year);
        festivalList.push({
          festivalName: this.festival.gregorian[key],
          date: key2list,
          dateString: year + '-' + key,
          dateTime: (new Date(+key2list[0], +key2list[1], +key2list[2])).getTime()
        });
      }
      // console.log(festivalList);
      festivalList.sort(function(a, b) {
        return a.dateTime - b.dateTime;
      });
      return festivalList;
    },

    filtrateFestivalList() {
      let beginTime = (new Date(+this.begin[0], +this.begin[1], +this.begin[2])).getTime();
      let endTime = (new Date(+this.end[0], +this.end[1], +this.end[2])).getTime();
      let inYearLastTiem = (new Date(this.year, 11, 31, 23, 59, 59)).getTime();
      let festivalList = this.handleFestivalList(+this.year);
      let festivalList2 = [];
      if (endTime > inYearLastTiem) {
        festivalList2 = this.handleFestivalList(+this.year + 1);
      }
      let dataList = festivalList.concat(festivalList2);
      let data = [];
      for (var j = 0, l = dataList.length; j < l; j++) {
        if (dataList[j].dateTime >= beginTime && dataList[j].dateTime <= endTime) {
          data.push(dataList[j]);
        }
      }
      this.$emit('FestivalList', data);
    }
  }
};
</script>
